...
path <- "//dapadfs/workspace_cluster_3/bid-cc-agricultural-sector/08-Cells_toRun/matrices_cultivo/version2017/"

# Load FPU shape file
worldFPU <- readShapePoly(fn = "//dapadfs/workspace_cluster_3/bid-cc-agricultural-sector/15_FPUs/fpu_shp/fpu.shp",proj4string=CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
...
for(c in 1:5){  #loop through crops
  for(t in 1:2){  #loop through treatments
    ...
    if(t==1){
      crop_riego = crop
      save(crop_riego, file=paste(path,cultivos[c],"_",treat[t],".RDat",sep=''))
    } else{
      crop_secano = crop
