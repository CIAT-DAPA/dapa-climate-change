## Code's Jeison
## CIAT, August 2015
## codigo para correr todos los genotipos de papa

path <- "/home/jeisonmesa/Proyectos/BID/DSSAT/bin/csm45_1_23_bin_ifort/"

sep <- c(7,17)
cultivars <- read.fwf(paste0(path, "PTSUB045.CUL"), sep, skip = 18, strip.white = T)
colnames(cultivars) <- c("Code", "Genotype")
pos_invalid <- grep("!", cultivars[, "Code"])
cultivars <- cultivars[-pos_invalid, ] 
...
load(paste0(path_project, "/08-Cells_toRun/matrices_cultivo/Potatoes_riego.RDat"))
...
day35 <- crop_riego$N.app.35d
day_aplication35 <- rep(35, length(day35))

amount <- data.frame(day0, day35)
day_app <- data.frame(day_aplication0, day_aplication35)


## configuracion Archivo experimental secano
years <- 71:99   ## Camabiar entre linea base 71:99 o futuro 69:94
data_xfile <- list()
data_xfile$crop <- "POTATO" 
data_xfile$exp_details <- "*EXP.DETAILS: BID17101RZ POTATO LAC"
data_xfile$name <- "./JBID.PTX" 
data_xfile$CR <- "PT"  ## Variable importante 
## data_xfile$INGENO <- "IB0118"
cultivars <- rep("IB0005", dim(crop_riego)[1])  ## Cambiar de acuerdo al cultivar a correr
data_xfile$INGENO <- cultivars  ## Correr Cultivar por region
data_xfile$CNAME <- "PTNA"
...
data_xfile$smodel <- "PTSUB045"     ##  Fin Model
data_xfile$bname <- "DSSBatch.v45"
data_xfile$PPOP <- 7   ## Investigar mas acerca de este parametro 
data_xfile$PPOE <- 7   ## Investigar mas acerca de este parametro 
...
#save(Run, file = paste("/home/jeisonmesa/Proyectos/BID/bid-cc-agricultural-sector/","_", cultivo,tipo,"WFD",".RDat",sep=""))
save(Run, file = paste("/home/jeisonmesa/Proyectos/BID/bid-cc-agricultural-sector/","_", cultivo,tipo,modelos[i],".RDat",sep=""))
