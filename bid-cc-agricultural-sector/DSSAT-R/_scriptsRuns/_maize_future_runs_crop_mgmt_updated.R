...
# Iterate GCM's
for (gcm_i in 7) { # 1:length(modelos)
  ...
  for (cultivar in 1:nrow(cul_list)) {
    ...
    load(paste0(path_project, "/08-Cells_toRun/matrices_cultivo/version2017/Maize_", sys_type, ".RDat"))
    ...
    if (run_type == "diagnostic") {
      ...
      if(!file.exists(paste(path_project, "/08-Cells_toRun/matrices_cultivo/version2017/_maize_crop_mgmt_", sys_type, ".Rdat", sep = ""))){
        ...
        crop_mgmt$SecondAppDay <- day_aplication40
        save(crop_mgmt, file = paste(path_project, "08-Cells_toRun/matrices_cultivo/version2017/_maize_crop_mgmt_", sys_type, ".RDat", sep = ""))
        ...
        amount <- data.frame(day0, day40)
        day_app <- data.frame(day_aplication0, day_aplication40)
        rm(day0, day40, day_aplication0, day_aplication40)
        
      } else {
        
        load(paste(path_project, "08-Cells_toRun/matrices_cultivo/version2017/_maize_crop_mgmt_", sys_type, ".RDat", sep = ""))
        day0 <-  crop_mgmt$N.app.0d
        day_aplication0 <- rep(0, length(day0))
        day0 <-  crop_mgmt$N.app.0d
        day40 <- crop_mgmt$N.app.40d
        ...
        amount <- data.frame(day0, day40)
        day_app <- data.frame(day_aplication0, day_aplication40=crop_mgmt$SecondAppDay)
        rm(day0, day40, day_aplication0)
    ...
    data_xfile$CR <- "MZ" # Variable importante 
    ...
    data_xfile$PPOP <- 7    # Plant population at planting
    data_xfile$PPOE <- 7    # Plant population at emergence
    ...
    data_xfile$PLRS <- 85   # Row spacing (cm)
    data_xfile$PLDP <- 5    # Planting depth (cm)
    ...
    if(!dir.exists(paste(path_project, "19-BID-reanalysis/Maize/", scenario, "/", run_type, sep = ""))){
      dir.create(paste(path_project, "19-BID-reanalysis/Maize/", scenario, "/", run_type, sep = ""), recursive = TRUE)
    }
    save(Run, file = paste(path_project, "19-BID-reanalysis/Maize/", scenario, "/", run_type, "/", store_name, ".RDat", sep = ""))
