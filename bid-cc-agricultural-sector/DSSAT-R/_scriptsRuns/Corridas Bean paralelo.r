...
## configuracion Archivo experimental secano
  ...
  data_xfile$INGENO <- rep('IB0010', length(crop_riego[, "variedad.1"]))
...
save(Run, file = paste("/home/jmesa/","_", cultivo, tipo, cultivar, 'WFD', ".RDat", sep = ""))
  ...
  load(paste0(path_project, "/08-Cells_toRun/matrices_cultivo/Bean_secano.RDat"))
  ...
  day20 <- rep(0, length(crop_secano$N.app.0))
  day_aplication20 <- rep(2, length(day20))
...
## configuracion Archivo experimental secano
  ...
  data_xfile$INGENO <- rep('IB0010', length(crop_secano[, "variedad.1"]))
  data_xfile$CNAME <- "BZNA"
  ...
  run_dssat(input_data, 2, dir_dssat, dir_base)
