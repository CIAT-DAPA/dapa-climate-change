...
#   filled.contour(lon,lat,plant,color=terrain.colors,asp=1)  #2nd option
  ...
  #eval(parse(text=paste('load("Z:/08-Cells_toRun/loc.cal_old/',crops[j],'.loc.cal.Rdat")',sep='')))
  eval(parse(text=paste('load("Z:/08-Cells_toRun/',crops[j],'.loc.cal.Rdat")',sep='')))  #nuevos matrices!!
  eval(parse(text=paste('crop.loc = ',crops[j],'.loc.cal',sep='')))  
...
mirca = read.table('Z:/09-MIRCA2000/CELL_SPECIFIC_CROPPING_CALENDARS_30MN.txt',sep='',header=T)
#UPDATE CROP NUMBER HERE (2/28 maize, 10/36 potato, 3/29 rice, 8/34 soy, 1/27 wheat, 17/43 pulses)
crop.irr = which((mirca$crop==1|mirca$subcrop==1) & mirca$lat>=(-56) & mirca$lat<=33 & mirca$lon>=(-117) & mirca$lon<=(-34))  #cut out Latin America (& bit of US)
crop.rf = which((mirca$crop==27|mirca$subcrop==27) & mirca$lat>=(-56) & mirca$lat<=33 & mirca$lon>=(-117) & mirca$lon<=(-34))  #cut out Latin America (& bit of US)
crop.irr = mirca[crop.irr,]
crop.rf = mirca[crop.rf,]

#select rows for same pixel with highest area
library(plyr)
crop.irr.sort = arrange(crop.irr,lat,lon,desc(area))  #order by area for each pixel
crop.rf.sort = arrange(crop.rf,lat,lon,desc(area))

#try matching these cells to criteria from Monfreda
#match uses 1st find for multiple finds
ind_mirca.irr = match(paste(round(crop.loc$Latitude,2),round(crop.loc$Longitude,2),sep=''),paste(round(crop.irr.sort$lat,2),round(crop.irr.sort$lon,2),sep=''))
ind_mirca.rf = match(paste(round(crop.loc$Latitude,2),round(crop.loc$Longitude,2),sep=''),paste(round(crop.rf.sort$lat,2),round(crop.rf.sort$lon,2),sep=''))

mirca.crop.irr = crop.irr.sort[ind_mirca.irr,]
mirca.crop.rf = crop.rf.sort[ind_mirca.rf,]
...
for (k in 1:dim(crop.loc)[1])  {
  if (is.na(crop.loc$mirca.irr.start[k])==F)  {
    if (crop.loc$mirca.irr.start[k]==1) {crop.loc$mirca.irr.start[k]=15}  else{  #January
      crop.loc$mirca.irr.start[k] = sum(days[1:(crop.loc$mirca.irr.start[k]-1)])+15
    }
    if (crop.loc$mirca.irr.end[k]==1) {crop.loc$mirca.irr.end[k]=15}  else{
      crop.loc$mirca.irr.end[k] = sum(days[1:(crop.loc$mirca.irr.end[k]-1)])+15
    }
  }

  if (is.na(crop.loc$mirca.rf.start[k])==F)  {
    if (crop.loc$mirca.rf.start[k]==1) {crop.loc$mirca.rf.start[k]=15}  else{
      crop.loc$mirca.rf.start[k] = sum(days[1:(crop.loc$mirca.rf.start[k]-1)])+15
    }
    if (crop.loc$mirca.rf.end[k]==1) {crop.loc$mirca.rf.end[k]=15}  else{
