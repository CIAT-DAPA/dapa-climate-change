...
eval(parse(text=paste("secano.phys.area = raster('/mnt/workspace_cluster_3/bid-cc-agricultural-sector/07-SPAM_data/SPAM2005/physical.area/spam2005v2r0_physical-area_",crops_spam[k],"_rainfed.nc')",sep='')))
...
load('/mnt/workspace_cluster_3/bid-cc-agricultural-sector/14-ObjectsR/id_coordinates.Rdat')  #coordinates of DSSAT runs by crop
#eval(parse(text=paste('crop_pixel = id_pixel[which(id_pixel$',crops[k],'==1),]',sep='')))

#match to SPAM data
ind_LAM_riego = match(paste(id_pixel$x,id_pixel$y),paste(round(riego.pts.p5[,'x'],2),round(riego.pts.p5[,'y'],2)))
riego.pts.LAM = riego.pts.p5[ind_LAM_riego,]

ind_LAM_secano = match(paste(id_pixel$x,id_pixel$y),paste(round(secano.pts.p5[,'x'],2),round(secano.pts.p5[,'y'],2)))
...
if (length(ind.bad)>0)  {crop_RS = crop_RS[-ind.bad,]}
...
eval(parse(text=paste("save(crop_RS,file='/mnt/workspace_cluster_3/bid-cc-agricultural-sector/07-SPAM_data/SPAM2005/physical.area/RS/",crops[k],"_RS.Rdat')",sep='')))

##COMPARE MIRCA & SPAM DATA FOR IRRIGATED/ RAINFED AREA
eval(parse(text=paste("load('/mnt/workspace_cluster_3/bid-cc-agricultural-sector/07-SPAM_data/SPAM2005/physical.area/RS/",crops[k],"_RS.Rdat')",sep='')))
spam=crop_RS
eval(parse(text=paste("load('/mnt/workspace_cluster_3/bid-cc-agricultural-sector/09-MIRCA2000/Growing_area_grids/RS/",crops[k],"_RS.Rdat')",sep='')))
mirca=crop_RS

#plot irrigated/ rainfed area for each dataset
Map_LatinAmerica<-shapefile("/mnt/workspace_cluster_3/bid-cc-agricultural-sector/03-Map_LatinAmerica/Latino_America.shp")
