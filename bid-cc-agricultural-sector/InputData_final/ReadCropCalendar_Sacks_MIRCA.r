...
#   filled.contour(lon,lat,plant,color=terrain.colors,asp=1)  #2nd option
  ...
  eval(parse(text=paste('load("Z:/08-Cells_toRun/',crops2[j],'.loc.cal.Rdat")',sep='')))
  eval(parse(text=paste('crop.loc = ',crops2[j],'.loc.cal',sep='')))  
...
crop.irr = which((mirca$crop==3|mirca$subcrop==3) & mirca$lat>=(-56) & mirca$lat<=33 & mirca$lon>=(-117) & mirca$lon<=(-34))  #cut out Latin America (& bit of US)
crop.rf = which((mirca$crop==29|mirca$subcrop==29) & mirca$lat>=(-56) & mirca$lat<=33 & mirca$lon>=(-117) & mirca$lon<=(-34))  #cut out Latin America (& bit of US)
