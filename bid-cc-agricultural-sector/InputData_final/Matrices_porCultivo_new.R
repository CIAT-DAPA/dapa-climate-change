...
# Apply this for all crops
lapply(1:length(crops), function(k){
  
  #Load SPAM matrices
  eval(parse(text=paste("load('", path, "/07-SPAM_data/SPAM2005/physical.area/RS/", crops[k], "_RS.Rdat')", sep='')))
  
  #Load pixel ID's & countries
  load(paste(path, '/14-ObjectsR/id_coordinates.Rdat', sep = ''))
  ...
  crop_RS$N.app.rf = crop_RS$N.app
  crop_RS$N.app.irr = crop_RS$N.app.rf
  ...
  #Add FPU
  # Load FPU shape file
  worldFPU <- readShapePoly(fn = paste(path, "/15_FPUs/fpu_shp/fpu.shp", sep = ""), proj4string=CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
  ...
  #Add elevation
  load(paste(path, '/01-climate-data/elevation/elevation_LAM.rdat', sep = ''))
  ...
  pixels.EC = read.csv(paste(path, '/08-Cells_toRun/matrices_cultivo/maiz_Ecuador_fechas.csv', sep = ''),header=T)
  ...
  #save results
  eval(parse(text=paste('save(crop_riego, file = "', path, '/08-Cells_toRun/matrices_cultivo/version2017/', crops[k], '_riego.Rdat")', sep='')))
  eval(parse(text=paste('save(crop_secano, file = "', path, '/08-Cells_toRun/matrices_cultivo/version2017/', crops[k], '_secano.Rdat")', sep='')))
  ...
  return(cat(paste("Process done for:", cultivos[k], "\n", sep = "")))
