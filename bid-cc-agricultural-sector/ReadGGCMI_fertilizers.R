...
fert = open.ncdf('agmip_maize_apprate_fill_NPK_0.5.nc4') #load entire netcdf file with 11 variables
...
range(K2O.app,na.rm=T)
cutpts = seq(0,400,by=40)
levelplot(K2O.app~lon*lat,data=grid,at=cutpts,cuts = 10,pretty=T,col.regions=(brewer.pal(10,'RdBu')))
