...
#params = c('HWAH','Stress_water1','IRCM')  #HWAH (yield) should always be the first
params = c('HWAH','PDAT','ADAT','MDAT','SDAT','IRCM')
...
wfd.fail.r = apply(yld_riego[,,(p-1),,1],c(1,3),function(x) sum(x<thresh|is.na(x)))  #multi-annual means
wfd.fail.s = apply(yld_secano[,,(p-1),,1],c(1,3),function(x) sum(x<thresh|is.na(x)))
...
# #hard-code a single variety instead
# yld_secano.2 = yld_secano  #make backup with varieties
# yld_riego.2 = yld_riego
# yld_secano = yld_secano.2[,,,3,]  #variety 1, 2 or 3
# yld_riego = yld_riego.2[,,,3,]
...
test = apply(yld_riego[,,10,which(params=='IRCM')],1,mean)  
ind.rain = which(test==0)
length(ind.rain)/dim(yld_riego)[1]*100  #check % of pixels, shouldn't be too high
if (length(ind.rain)>0) {yld_riego = yld_riego[-ind.rain,,,]
crop_riego = crop_riego[-ind.rain,]}

# #pull out simulation start date (IS THIS NEEDED?)
# yld_secano[,,10,5] = as.numeric(substr(yld_secano[,,10,5],5,8))
# yld_secano[,,10,2] = as.numeric(substr(yld_secano[,,10,2],5,8))

#pull out planting and anthesis dates; calculate length of vegetative and grain-filling stages
yld_secano[,,,2] = as.numeric(substr(yld_secano[,,,2],5,8))  #planting date in calendar days
yld_secano[,,,3] = as.numeric(substr(yld_secano[,,,3],5,8))  #anthesis date
yld_secano[,,,4] = as.numeric(substr(yld_secano[,,,4],5,8))  #maturity date
yld_secano[,,,5] = as.numeric(substr(yld_secano[,,,5],5,8))  #simulation start date
veg.days.s = yld_secano[,,,3] - yld_secano[,,,2]
veg.days.s[which(is.na(veg.days.s)==F & veg.days.s<0)] = veg.days.s[which(is.na(veg.days.s)==F & veg.days.s<0)]+365
gf.days.s = yld_secano[,,,4] - yld_secano[,,,3]
gf.days.s[which(is.na(gf.days.s)==F & gf.days.s<0)] = gf.days.s[which(is.na(gf.days.s)==F & gf.days.s<0)]+365

yld_riego[,,,2] = as.numeric(substr(yld_riego[,,,2],5,8))  #planting date in calendar days
yld_riego[,,,3] = as.numeric(substr(yld_riego[,,,3],5,8))  #anthesis date
yld_riego[,,,4] = as.numeric(substr(yld_riego[,,,4],5,8))  #maturity date
yld_riego[,,,5] = as.numeric(substr(yld_riego[,,,5],5,8))  #simulation start date
veg.days.r = yld_riego[,,,3] - yld_riego[,,,2]
veg.days.r[which(is.na(veg.days.r)==F & veg.days.r<0)] = veg.days.r[which(is.na(veg.days.r)==F & veg.days.r<0)]+365
gf.days.r = yld_riego[,,,4] - yld_riego[,,,3]
gf.days.r[which(is.na(gf.days.r)==F & gf.days.r<0)] = gf.days.r[which(is.na(gf.days.r)==F & gf.days.r<0)]+365

#add growth durations to yld_secano & riego
library(abind)
yld_secano = abind(yld_secano,veg.days.s)
yld_riego = abind(yld_riego,veg.days.r)
yld_secano = abind(yld_secano,gf.days.s)
yld_riego = abind(yld_riego,gf.days.r)
params = c(params,'veg.days','gf.days')

#shift planting & anthesis dates when they cross calendar year so that multi-annual means & differences make sense
for (j in 1:dim(crop_secano)[1]) { #loop by pixel
  pdat.window = range(yld_secano[j,,,2],na.rm=T)  #find range of calendar days
  if ((pdat.window[2]-pdat.window[1])>98) {  #if window crosses calendar year...
    pixel.p = yld_secano[j,,,2]
    pixel.p[which(pixel.p<120)] = pixel.p[which(pixel.p<120)]+365  #shift early dates to following year
    yld_secano[j,,,2] = pixel.p
  }
}

for (j in 1:dim(crop_secano)[1]) { #loop by pixel
  adat.window = range(yld_secano[j,,,3],na.rm=T)  #find range of calendar days
  if ((adat.window[2]-adat.window[1])>250) {  #if window crosses calendar year...
    pixel.p = yld_secano[j,,,3]
    pixel.p[which(pixel.p<150)] = pixel.p[which(pixel.p<150)]+365  #shift early dates to following year
    yld_secano[j,,,3] = pixel.p
  }
}

for (j in 1:dim(crop_riego)[1]) { #loop by pixel
  adat.window = range(yld_riego[j,,,3],na.rm=T)  #find range of calendar days
  if ((adat.window[2]-adat.window[1])>250) {  #if window crosses calendar year...
    pixel.p = yld_riego[j,,,3]
    pixel.p[which(pixel.p<150)] = pixel.p[which(pixel.p<150)]+365  #shift early dates to following year
    yld_riego[j,,,3] = pixel.p
  }
}
...
save(wfd.r,wfd.s,file=paste(path.root,"resultados_graficas/",cultivos[c],"_WFD_",params[var.g],".Rdat", sep=""))
 
#check how much before MIRCA for planting date
if (params[var.g]=='PDAT') {
  test = wfd.s
  test[test>365] = test[test>365]-365
  gap = test - crop_secano$mirca.start
  quantile(gap)  
}
...
if (params[var.g]=='PDAT'|params[var.g]=='ADAT'|params[var.g]=='TMINA'|params[var.g]=='TMAXA'|params[var.g]=='Stress_water_all'|params[var.g]=='Stress_nitrogen_all') {  
...
pct.ch.r2 = cbind(pct.ch.r2,crop_riego$riego.area)  #add pixel-wise areas
pct.ch.s2 = cbind(pct.ch.s2,crop_secano$secano.area)

pct.ch_cols = c('PctCh.MMM','SigCh.MMM','PctCh.lower','PctCh.upper','Area')
...
#save vectors to compare across crops
save(pct.ch.r2,file=paste(path.root,"resultados_graficas/",cultivos[c],"_riego_pctCh_",params[var.g],".Rdat", sep=""))
save(pct.ch.s2,file=paste(path.root,"resultados_graficas/",cultivos[c],"_secano_pctCh_",params[var.g],".Rdat", sep=""))
...
for (t in 1:2)  {  #loop through riego (t=1)/ secano (t=2)
  ...
  for (m in c((p+1)))  {  #(p-1):(p+1):  WFD, multi-model mean y % cambio (4 tipos)
    ...
    if(m<=p)  {  #for models + WFD
      ...
      if (params[var.g]=='ADAT'|params[var.g]=='PDAT') {
        limits2 = c(-14,14)  #rend y WUE
      } 
      if (params[var.g]=='NDCH') {
    ...
    y=ggplot() +
      geom_polygon( data=Map_LatinAmerica1, aes(x=long, y=lat, group = group),colour="gray90", fill="white" )+
      ...
      scale_fill_gradientn(colours=color_scale,limits=limits2,na.value = "gray85",guide='none')+ # limits ,breaks=as.vector(limits),labels=as.vector(limits),limits=as.vector(limits)
      theme_bw()+
      labs(fill=labs2)+
