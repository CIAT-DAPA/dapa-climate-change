...
#params = c('HWAH','Stress_water_all','IRCM')  #HWAH (yield) should always be the first
params = c('HWAH','H.AM','IRCM','CWAM','TMAXA','TMINA')
#params = c('HWAH','NDCH','IRCM','PRCP','SRADA')
...
yld_riego[yld_riego==-99|yld_riego==-99.9|yld_riego==9999999] = NA  #(yields for crop failures will still be 0)
...
var.g = 1  #var.g = 4 is yield/IRCM for irrigation productivity
...
if (yld.na==T & params[var.g]=='HWAH') {
  save(wfd.r,wfd.s,file=paste(path.root,"resultados_graficas/",cultivos[c],"_WFD_",params[var.g],"_NA.Rdat", sep=""))
}  else { 
  if (yld.na==F & params[var.g]=='HWAH') {
    save(wfd.r,wfd.s,file=paste(path.root,"resultados_graficas/",cultivos[c],"_WFD_",params[var.g],"_zeros.Rdat", sep=""))
  }  else {
...
if (yld.na==T & params[var.g]=='HWAH') {
  save(pct.ch.r2,file=paste(path.root,"resultados_graficas/",cultivos[c],"_riego_pctCh_",params[var.g],"_NA.Rdat", sep=""))
  save(pct.ch.s2,file=paste(path.root,"resultados_graficas/",cultivos[c],"_secano_pctCh_",params[var.g],"_NA.Rdat", sep=""))
  
}  else { 
  if (yld.na==F & params[var.g]=='HWAH') {
    save(pct.ch.r2,file=paste(path.root,"resultados_graficas/",cultivos[c],"_riego_pctCh_",params[var.g],"_zeros.Rdat", sep=""))
    save(pct.ch.s2,file=paste(path.root,"resultados_graficas/",cultivos[c],"_secano_pctCh_",params[var.g],"_zeros.Rdat", sep=""))
  ...
  for (m in c(p+1))  {  #(p-1):(p+1):  WFD, multi-model mean y % cambio (4 tipos)
    ...
    if(m<=p)  {  #for models + WFD
      ...
      if (params[var.g]=='HWAH') {
        ...
        if (c==1|c==2|c==5) {limits2 = c(0.9,10.1)}  else {limits2 = c(0.7,5)}  #limites fijados
      } 
