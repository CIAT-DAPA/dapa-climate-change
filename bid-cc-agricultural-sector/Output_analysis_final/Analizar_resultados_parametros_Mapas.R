...
params = c('NDCH','HWAH','TMAXA','TMINA','CO2A','Stress_water_all')  #,'PDAT','EPCM')  #duración, rendimiento y otro variables
...
yld_secano[yld_secano==-99|yld_secano==9999999] = NA  #re-emplazar -99 con NA
yld_riego[yld_riego==-99|yld_riego==9999999] = NA
...
yld_secano[yld_secano[,,,1]>250]=NA
...
# #Process planting date info (if exists)
# ind.p = which(params=='PDAT')
# if (length(ind.p)==1)  {
#   PDAT.s = yld_secano[,,,ind.p]
#   PDAT.s = as.numeric(substr(PDAT.s,5,7))
#   PDAT.r = yld_riego[,,,ind.p]
#   PDAT.r = as.numeric(substr(PDAT.r,5,7))
#   yld_secano[,,,ind.p] = PDAT.s  #re-emplazar fecha de siembra
#   yld_riego[,,,ind.p] = PDAT.r
# }

#Graficas comparando variables
x.var = 5  #cual variable quieres graficar?
...
plot(yld_secano[,,,x.var],yld_secano[,,,2],xlab='CO2',ylab='Yield',xlim=xlim.c,ylim=ylim.c)
test = lm(c(yld_secano[,,,2])~c(yld_secano[,,,x.var])+eval(c(yld_secano[,,,x.var]^2)),x=T)
...
plot(yld_riego[,,10,x.var],yld_riego[,,10,2],xlab='Duration',ylab='Yield',xlim=xlim.c,ylim=ylim.c)
test = lm(c(yld_riego[,,10,2])~c(yld_riego[,,10,x.var]),x=T)
...
for (t in 1:1)  {  #loop through riego (t=1)/ secano (t=2)
  #t=2  #secano
  print(t)
  for (m in c((p-1):(p+1)))  {  #(p-1):(p+1):  WFD, multi-model mean y % cambio (4 tipos)
    ...
    #plot(y)
    ggsave(filename=paste(path.root,"resultados_graficas/",cultivos[c],"_",treat[t],"_",models[m,],"_",params[var.g],".png", sep=""), plot=y, width=5, height=5, dpi=400,scale=1.5)
  }
}  
