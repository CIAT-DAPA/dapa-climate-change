...
for (j in 1:6)  {
  ...
  ## Load Monfreda data
  eval(parse(text=paste('area.prop<-raster("06-Monfreda data (Rendimiento, Area)/',cultivos[j],'/',cultivos[j],'_5min.nc",level=1)',sep='')))  
  area.prop.LA<-crop(area.prop,Map_LatinAmerica)  ##harvested area as proportion of grid cell area
  
  eval(parse(text=paste('rend<-raster("06-Monfreda data (Rendimiento, Area)/',cultivos[j],'/',cultivos[j],'_5min.nc",level=2)',sep='')))  #harvested area, proportion of grid cell area
  ...
  prod.LA = rend.LA * area.LA  #calculate production as yield * harvested area
  ## plot(area.LA,main=cultivos[j])  #try graphing harvested area map (in hectares)
  
  ## Agregar a la resolucion del 0.5 grados (de 5 minutos o 0.08 grados?) por sumar
