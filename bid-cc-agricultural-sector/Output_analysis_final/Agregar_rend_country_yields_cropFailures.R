...
pct.ch.country.CI = apply(boot,2,function(x) quantile(x,c(.05,.95)))
colnames(pct.ch.country.CI) = countries

#Save results 
eval(parse(text=paste("save(yld.country.m,file='D:/tobackup/BID/FAOSTAT_validation/ylds_",cultivos[c],"_optVariety.Rdat')",sep="")))
eval(parse(text=paste("save(pct.ch.country,pct.ch.country.CI,file='D:/tobackup/BID/FAOSTAT_validation/country.pctCh_",cultivos[c],"_optVariety.Rdat')",sep="")))
...
barplot2(pct.ch.country[ind.sort],width=0.8,xlim=c(0,38),col=regions[ind.sort],yaxt='n',xaxt='n',ylim=c(-55,55),xlab='',space=1,plot.ci=T,ci.l=pct.ch.country.CI[1,ind.sort],ci.u = pct.ch.country.CI[2,ind.sort])
...
fail.future.CI = apply(boot,2,function(x) quantile(x,c(.05,.95)))
...
#Save results 
eval(parse(text=paste("save(fail.compare,fail.future.CI,file='D:/tobackup/BID/FAOSTAT_validation/country.areaFail_",cultivos[c],"_optVariety.Rdat')",sep="")))
...
#create barplot
barplot2(t(fail.compare[ind.sort,]),beside=T,ylim=c(0,30),density=c(NA,50),col=rbind(regions[ind.sort],regions[ind.sort]),yaxt='n',xaxt='n',xlab='',plot.ci=T,ci.l=rbind(NA,fail.future.CI[1,ind.sort]),ci.u=rbind(NA,fail.future.CI[2,ind.sort]))
...
if (length(countries.2)==1) {
  barplot2(cbind(NA,fail.compare.2),width=0.8,xlim=c(0,48),beside=T,ylim=c(0,40),density=c(NA,40),col=rbind(cols,cols),yaxt='n',xaxt='n',xlab='',plot.ci=T,ci.l=cbind(NA,c(NA,fail.future.CI.2[1])),ci.u=cbind(NA,c(NA,fail.future.CI.2[2])))
} else{
  barplot2(t(fail.compare.2[ind.sort3,]),width=0.8,xlim=c(0,48),beside=T,ylim=c(0,40),density=c(NA,40),col=rbind(cols,cols),yaxt='n',xaxt='n',xlab='',plot.ci=T,ci.l=rbind(NA,fail.future.CI.2[1,ind.sort3]),ci.u=rbind(NA,fail.future.CI.2[2,ind.sort3]))
}
...
pct.ch.FPU.CI = apply(boot,2,function(x) quantile(x,c(.05,.95)))
colnames(pct.ch.FPU.CI) = FPUs

#Save results 
eval(parse(text=paste("save(pct.ch.FPU,pct.ch.FPU.CI,file='D:/tobackup/BID/FAOSTAT_validation/country.pctCh_",cultivos[c],"_optVariety_FPU.Rdat')",sep="")))
...
#Save results 
eval(parse(text=paste("save(fail.compare,fail.future.CI,file='D:/tobackup/BID/FAOSTAT_validation/regions.areaFail_",cultivos[c],"_optVariety.Rdat')",sep="")))

#Color-code regions
colors = c('darkorange','yellow','green2','royalblue3','red3')

#create barplot
barplot2(t(fail.compare),beside=T,ylim=c(0,20),density=c(NA,50),col=rbind(colors,colors),yaxt='n',xaxt='n',xlab='',plot.ci=T,ci.l=rbind(NA,fail.future.CI[1,]),ci.u=rbind(NA,fail.future.CI[2,]))
