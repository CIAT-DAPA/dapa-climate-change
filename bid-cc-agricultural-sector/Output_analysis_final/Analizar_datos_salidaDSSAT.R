...
#start with year 1!
for (m in c(1,2)) {
  prom.secano = apply(maiz_secano[,,m],1,mean,na.rm=T)  #average in zeros??
  cols = col_pal[cut(prom.secano,data_seq,include.lowest=T)]
  cols[which(prom.secano==0)]='#FF1493'  #poner zero como rosada
  plot(Map_LatinAmerica,xlim=c(-116,-35),ylim=c(-54,32))
  points(crop_secano[,1:2], col = cols, pch=16, cex=0.5)
  #title('Maize (secano):  Linea Base')
  title(paste('Maiz (secano): ',models[1],sep=''))
  
  prom.riego = apply(maiz_riego[,,m],1,mean,na.rm=T)
  cols = col_pal[cut(prom.riego,data_seq,include.lowest=T)]
  cols[which(prom.riego==0)]='#FF1493'  #poner zero como rosada
  plot(Map_LatinAmerica,xlim=c(-116,-35),ylim=c(-54,32))
  points(crop_riego[,1:2], col = cols, pch=16, cex=0.5)
  #title('Maize (riego):  Linea Base')
  title(paste('Maiz (riego): ',models[m],sep=''))
}
