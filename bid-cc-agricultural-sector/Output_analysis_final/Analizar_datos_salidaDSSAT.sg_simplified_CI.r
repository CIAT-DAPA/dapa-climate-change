...
path.root = "D:/tobackup/BID/"; #'D:/BID/'  #root path
carp.res.riego = 'Soya-Riego_22julio2015_ENTREGA'  #nombre de carpeta con los resultados
carp.res.secano = 'Soya-Secano_23julio2015_ENTREGA'
...
#repeat wfd for 10 models
wfd.r2 = replicate(10,wfd.r)
wfd.s2 = replicate(10,wfd.s)
...
pct.ch.s2[,3] = apply(pct.ch.s,1,mean)
pct.ch.r2[,3] = apply(pct.ch.r,1,mean)
mmm.s = apply(models.s,1,mean)
mmm.r = apply(models.r,1,mean)
...
for (t in 1:2)  {  #loop through riego (t=1)/ secano (t=2)
  ...
  for (m in (p-1):(p+1))  {  #(p-1):(p+1):  WFD, multi-model mean y % cambio
    ...
    if (m<=(p-1))  {
      eval(parse(text=paste('promedio = apply(yld_',treat[t],'[,,m],1,mean,na.rm=T)',sep='')))  #average in zeros??  
        ...
        eval(parse(text=paste('promedio = mmm.',substr(treat[t],1,1),sep='')))
          ...
          eval(parse(text=paste('promedio = pct.ch.',substr(treat[t],1,1),'2[,4]',sep='')))  #use column 3 for all changes, and 4 for only significant changes
        }  
      }
    }
    ...
    #plot(y)
    ggsave(filename=paste(path.root,"resultados_graficas/",cultivos[c],"_",treat[t],"_",models[m,],".png", sep=""), plot=y, width=5, height=5, dpi=400,scale=1.5)
  }
}  
