...
carp.res.riego = 'Arroz-6-Noviembre_noCO2'
carp.res.secano = 'Arroz-6-Noviembre_noCO2'
...
anos = 2:25  #fijar años para analizar aquí
  ...
  for (j in 1:length(Run.secano))  {
    if (is.null(dim(Run.secano[[j]])))  {
      ...
      if (m==10) {
        ...
        ind.s = as.numeric(substr(Run.secano[[j]][,1],1,4))-1968
      }
  ...
  for (j in 1:length(Run.riego))  {
    if (is.null(dim(Run.riego[[j]])))  {
      ...
      if (m==10) {
        ...
        ind.r = as.numeric(substr(Run.riego[[j]][,1],1,4))-1968
      }
...
apply(yld_secano[,,1],2,function(x) sum(is.na(x)==F))
...
ind.bad.r = which(riego.fallas[,10]>12)
ind.bad.s = which(secano.fallas[,10]>12)
...
for (t in 1:2)  {  #loop through riego (t=1)/ secano (t=2)
  ...
  for (m in c((p-1):(p+1)))  {  #(p-1):(p+1):  WFD, multi-model mean y % cambio (4 tipos)
    ...
    if(m<=p)  {  #for models + WFD
      ...
      limits2 = quantile(c(yld_riego,yld_secano),c(.05,.95),na.rm=T)  #limites flexibles segun el cultivo
      #limits2 = quantile(c(wfd.s,wfd.r,mmm.s,mmm.r),c(.05,.95),na.rm=T)
      limits2 = c(0,9000)  #limites fijados
...
#         limits2 = c(limits2,-1*limits2)  #set positive equal to opposite of negative
    ...
    #plot(y)
    ggsave(filename=paste(path.root,"resultados_graficas/",cultivos[c],"_",treat[t],"_",models[m,],".optimalVariety.png", sep=""), plot=y, width=5, height=5, dpi=400,scale=1.5)  #_documentos/graficas
  }
}  
...
for (t in 1:2)  {  #loop by treatment
  for (r in 1:length(FPUs))  {  #then by FPU
   eval(parse(text=paste("ind.reg = which(crop_",treat[t],"$New_FPU==FPUs[r] & zeros.wfd.",substr(treat[t],1,1),"<=12)",sep='')))
...
#multi-model means
FPU.pctCh.mmm.r = apply(FPU.pctCh.r,2,mean)
FPU.pctCh.mmm.s = apply(FPU.pctCh.s,2,mean)
FPU.CVCh.mmm.r = apply(FPU.CVCh.r,2,mean)
FPU.CVCh.mmm.s = apply(FPU.CVCh.s,2,mean)
...
rownames(yld.country) = 1972:1995    
...
#Save results to csv
eval(parse(text=paste("write.csv(yld.country,file='D:/tobackup/BID/FAOSTAT_validation/country.ylds_",cultivos[c],".csv')",sep="")))
...
CV.ch.r.mmm = apply(CV.ch.r,1,mean)
CV.ch.s.mmm = apply(CV.ch.s,1,mean)
...
for (j in 1:6)  {  #5 regions + all
  for (t in 1:2)  {  #loop through riego/ secano
    ...
    #subset regions
    if (j<=5) {eval(parse(text=paste("ind.reg = which(crop_",treat[t],"$Region==regions[j] & zeros.wfd.",substr(treat[t],1,1),"<=12)",sep='')))
    }   else{
