...
#Save results to csv
eval(parse(text=paste("write.csv(yld.country,file='D:/tobackup/BID/FAOSTAT_validation/country.ylds_",cultivos[c],"_optVariety.csv')",sep="")))
...
for (t in 1:2)  {
  for (r in 1:6)  {
    #subset regions
    if (r<=5) {eval(parse(text=paste("ind.reg = which(crop_",treat[t],"$Region==regions[r])",sep='')))
    }   else{
...
#put results in data frame and save
eval(parse(text=paste(cultivos[c],".fail.r = area.fail.r ",sep='')))
eval(parse(text=paste(cultivos[c],".fail.s = area.fail.s ",sep='')))
eval(parse(text=paste("save(",cultivos[c],".fail.r,file='",path.res,"summaries/",cultivos[c],".fail.r.Rdat')",sep="")))
eval(parse(text=paste("save(",cultivos[c],".fail.s,file='",path.res,"summaries/",cultivos[c],".fail.s.Rdat')",sep="")))
