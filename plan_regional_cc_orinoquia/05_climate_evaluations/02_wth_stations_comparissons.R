...
# Observados
st <- read.csv(paste0(iDir, "/rain_lla.csv"))
prec.st <- st[which(st$LONG >= xt@xmin & st$LONG <= xt@xmax & st$LAT >= xt@ymin & st$LAT <= xt@ymax),]
st <- read.csv(paste0(iDir, "/tmax_lla.csv"))
tmax.st <- st[which(st$LONG >= xt@xmin & st$LONG <= xt@xmax & st$LAT >= xt@ymin & st$LAT <= xt@ymax),]
st <- read.csv(paste0(iDir, "/tmin_lla.csv"))
tmin.st <- st[which(st$LONG >= xt@xmin & st$LONG <= xt@xmax & st$LAT >= xt@ymin & st$LAT <= xt@ymax),]

#Ajustados
stk <- stack(paste0(aDir, "/prec_",1:12,".asc"))
prec.adj <- extract(stk, prec.st[,6:7])
stk <- stack(paste0(aDir, "/tmax_",1:12,".asc"))
tmax.adj <- extract(stk, prec.st[,6:7])
stk <- stack(paste0(aDir, "/tmin_",1:12,".asc"))
tmin.adj <- extract(stk, prec.st[,6:7])

# WorldClim 
stk.wcl <- stack(paste0(wDir, "/prec_",1:12,".tif"))  
prec.wcl <- extract(stk.wcl, prec.st[,6:7])
stk.wcl <- stack(paste0(wDir, "/tmax_",1:12,".tif"))  
tmax.wcl <- extract(stk.wcl, prec.st[,6:7])
stk.wcl <- stack(paste0(wDir, "/tmin_",1:12,".tif"))  
tmin.wcl <- extract(stk.wcl, prec.st[,6:7])


for(i in 229:229){
  
  val_prec <- rbind(unlist(prec.adj[i,]), unlist(prec.st[i,9:20]), unlist(prec.wcl[i,]))
  val_prec1 <- data.frame(val_prec)
  names(val_prec1) <- mths
  prec <- rbind(unlist(val_prec1[1,]),unlist(val_prec1[2,]),unlist(val_prec1[3,]))
  
  png(paste(oDir, "/wth_stations/anncycle_llanos_", prec.st$OLD_ID[i], ".png",sep=""),width = 900, height = 600) #, pointsize = 15, compression="lzw")
  
  # windows()
  par(mar=c(8,5,1,5))
  barplot(prec,ylim=c(0,900),bes=T,axes=F,col=c("blue","skyblue","cyan"))
  
  axis(c(1,900)  			 #primero el intervalo que tiene que cubrir
       ,at=c(seq(0,900,100))			#donde queremos las marcas
       ,labels=c(seq(0,900,100))			#que numeros pone R en las marcas
       ,side=2					#en que lado
  ...
  par(new=T)
  plot(xlim=c(1,12),ylim=c(0,38),1:12, tmax.st[which(tmax.st$OLD_ID == paste0(prec.st$OLD_ID[i])),][,9:20], col="red",type="l",lwd=2,yaxt="n",ylab="",xaxt="n",xlab="")
  par(new=T)
  plot(xlim=c(1,12),ylim=c(0,38),1:12,tmax.adj[i,1:12],col="red",type="l",lwd=2,yaxt="n",ylab="",xaxt="n",xlab="",lty=2)
  par(new=T)
  plot(xlim=c(1,12),ylim=c(0,38),1:12,tmax.wcl[i,1:12]/10,col="red",type="l",lwd=2,yaxt="n",ylab="",xaxt="n",xlab="",lty=3)
  
  par(new=T)
  plot(xlim=c(1,12),ylim=c(0,38),1:12, tmin.st[which(tmin.st$OLD_ID == paste0(prec.st$OLD_ID[i])),][,9:20], col="orange",type="l",lwd=2,yaxt="n",ylab="",xaxt="n",xlab="")
  par(new=T)
  plot(xlim=c(1,12),ylim=c(0,38),1:12,tmin.adj[i,1:12],col="orange",type="l",lwd=2,yaxt="n",ylab="",xaxt="n",xlab="",lty=2)
  par(new=T)
  plot(xlim=c(1,12),ylim=c(0,38),1:12,tmin.wcl[i,1:12]/10,col="orange",type="l",lwd=2,yaxt="n",ylab="",xaxt="n",xlab="",lty=3)
  
  axis(c(1,38),at=c(seq(1,38,6)),labels=c(seq(1,38,6)),side=4,outer=F,las=2, cex=2)
  mtext(c("Precipitación (mm)"),side=2,line=3,at=c(19),cex = 1.5)
  
  par(xpd=T)
  text(13.1,19,c("Temperatura (°C)"),srt=270, cex = 1.5)
  
  legend("bottom",inset=-0.22,cex=0.9,ncol=3,legend=c("Temp_max estimada","Temp_max observada","Temp_max Worldclim","Temp_min estimada","Temp_min observada","Temp_min Worldclim","Prec. estimada","Prec. observada","Prec. Worldclim"),col=c("red","red","red","orange","orange","orange","blue","skyblue","cyan"),lwd=2,lty=c(2,1,3,2,1,3,2,1,1))
