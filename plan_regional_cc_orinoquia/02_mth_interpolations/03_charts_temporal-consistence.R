...
bDir <- "X:/ALPACAS/Plan_Regional_de_Cambio_Climatico_Orinoquia/01-datos_clima/baseline/tropico/outputs"
...
msk <- raster(paste("X:/ALPACAS/Plan_Regional_de_Cambio_Climatico_Orinoquia/01-datos_clima/baseline/tropico/_region/tile-1/altitude.asc", sep=""))
xt <- extent(msk)
xt@xmin <- xt@xmin #- 75
xt@xmax <- xt@xmax #- 71
xt@ymin <- xt@ymin #+ 1
...
tiff(paste0(oDir, '/skill_interpolation_metrics_tropico.tif'), width=1300, height=900, pointsize=8, compression='lzw',res=150)
...
par(mar=c(4,4,2,1))
boxplot(id_tmax$RMSE.TEST/10 ~ id_tmax$MONTH,xlab='Mes',ylab='ECM (ºC)',pch=20,cex=1.1,ylim=c(0,rmse_tmin_max), names=(mth_lb), cex.axis=0.9, col="gray90")
...
# tiff('fig-3f-rmse_tmean_monthly.tif',width=600, height=600,pointsize=8,compression='lzw',res=150)
par(mar=c(4,4,2,1))
boxplot(id_tmin$RMSE.TEST/10 ~ id_tmin$MONTH,xlab='Mes',ylab='ECM (ºC)',pch=20,cex=1.1,ylim=c(0,rmse_tmin_max), names=(mth_lb), cex.axis=0.9, col="gray90")
# text(x=11.9, y=0.98,"F", pos=4, cex=1.1.4)
grid()
