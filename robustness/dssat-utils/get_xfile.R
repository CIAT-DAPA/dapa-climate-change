...
#grab xfile details
get_xfile <- function(run_data, xfil_data=NULL) {
  basename <- run_data$BASENAME
  years <- run_data$ISYR:run_data$IEYR
  
  #if xfil_data is NULL then 
  if (is.null(xfil_data)) {
    #prepare object
    xfil_data <- list()
    ...
    xfil_data$fields$ID_FIELD <- paste(basename,"0001",sep="")
    xfil_data$fields$WSTA <- paste(basename,substr(as.character(run_data$ISYR),3,4),length(years),sep="")
    xfil_data$fields$XCRD <- run_data$LON
    xfil_data$fields$YCRD <- run_data$LAT
    xfil_data$fields$ELEV <- run_data$ELEV
    xfil_data$ini_cond_properties$ICDAT <- paste(substr(as.character(run_data$ISYR),3,4),sprintf("%03d",run_data$SOW_DATE),sep="")
    xfil_data$planting$PDATE <- paste(substr(as.character(run_data$ISYR),3,4),sprintf("%03d",run_data$SOW_DATE),sep="")
    xfil_data$planting$EDATE <- paste(substr(as.character(run_data$ISYR),3,4),sprintf("%03d",run_data$SOW_DATE+8),sep="")
    xfil_data$sim_ctrl$NYERS <-  length(years)-1 #run 1 year less in case of running short of weather years
    xfil_data$sim_ctrl$SDATE <- paste(substr(as.character(run_data$ISYR),3,4),sprintf("%03d",run_data$SOW_DATE),sep="")
    xfil_data$auto_mgmt$PFRST <- paste(substr(as.character(run_data$ISYR),3,4),sprintf("%03d",run_data$SOW_DATE),sep="")
    xfil_data$auto_mgmt$PLAST <- paste(substr(as.character(run_data$ISYR),3,4),sprintf("%03d",run_data$SOW_DATE+run_data$SOW_WINDOW),sep="")
  }
