...
for (tloc in loc_list) {
  #tloc <- loc_list[1]
  save_file <- paste(me_hdir,"/calib-",hrun,"_loc-",tloc,".RData",sep="")
...
if (length(loc_sel) > 0) {
  ...
  sfExport(list=c("me_sel","param_list","src.dir","me_hdir","wd","hyp_params","run_hyp_loc"))
  sfExport(list=c("driver","hrun"))
