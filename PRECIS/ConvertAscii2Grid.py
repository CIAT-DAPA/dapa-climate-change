...
if len(sys.argv) < 6:
	...
	print "\n Too few args"
	print "   - Sintaxis: "
	print "   - python ConvertAscii2Grid.py U:\rcm\precis\post_processed\sres_a2\echam4 2073 2097 daily U:\rcm\precis\post_processed\sres_a2\echam4"
	sys.exit(1)
...
if type == "daily":
	...
	for year in range(inityear, finalyear + 1, 1):
		...
		if not os.path.exists(dirbase + "\\" + type + "_grids\\" + str(year) + ".zip"):
			...
			if os.path.exists(dirbase + "\\" + type + "_asciis\\" + str(year)) and not gp.Exists(dirbase + "\\" + type + "_grids\\Ascii2Grid_" + str(year) + "_done.txt"):
				...
				if not os.path.exists(dirout + "\\" + type + "_grids\\" + str(year)):
					...
					for asc in ascList:
						...
						if not str(variable) == "08223" and not str(variable) == "08225" and not str(variable) == "16204":
			...
			for ascday in ascdayList:
				if not gp.Exists(dirout + "\\" + type + "_grids\\" + str(year) + "\\Ascii2Grid_" + str(os.path.basename(ascday)[:-9]) + "_done.txt"):					
