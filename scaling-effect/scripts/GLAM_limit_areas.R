...
pdf(paste(figDir,"/maize_yield_monfreda.pdf",sep=""), height=4,width=5,pointsize=16,family="Helvetica")
...
tplot <- tplot + layer(panel.text((m1@xmin+m1@xmax)*.5, (m1@ymin+m1@ymax)*.5, "M",cex=1.25))
print(tplot)
dev.off()
...
pdf(paste(figDir,"/maize_yield_glam.pdf",sep=""), height=4,width=5,pointsize=15,family="Helvetica")
...
pdf(paste(figDir,"/maize_yield_glam_ygp050.pdf",sep=""), height=4,width=5,pointsize=15,family="Helvetica")
tplot <- rs_levplot2(gyield,zn=NA,zx=NA,nb=NA,brks=seq(0,10,by=0.5),scale="Spectral",col_i="red",col_f="#FEE0D2",ncol=11,rev=F,leg=T)
tplot <- tplot + layer(sp.polygons(as(m1,'SpatialPolygons'),lwd=1.25,col="blue"))
tplot <- tplot + layer(panel.text((m1@xmin+m1@xmax)*.5, (m1@ymin+m1@ymax)*.5, "M",cex=1.25))
print(tplot)
