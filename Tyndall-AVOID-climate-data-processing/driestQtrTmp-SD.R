...
driQtrTmp <- function(rlist, rlist2, outfile, outfileSD, format='') {
	...
	if (!file.exists(outfile) | !file.exists(outfileSD)) {
			...
			p9sd <- raster(PpTaStack, 0)
			
			filename(p9) <- outfile
			filename(p9sd) <- outfileSD
			...
			for (rw in 1:nrow(PpTaStack)) {
				...
				RasVals <- apply(rowVals, 1, p9funsd)
				p9sd <- setValues(p9sd, RasVals, rw)
				p9sd <- writeRaster(p9sd, outfileSD, format=format, overwrite=TRUE)
