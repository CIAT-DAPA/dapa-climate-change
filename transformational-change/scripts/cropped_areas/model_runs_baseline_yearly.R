...
#working dir
bdir <- paste(nfs_base,"/Leeds-work",sep="")
wd <- paste(bdir,"/cul-de-sacs",sep="")
...
#source model
src_dir <- "~/Repositories/dapa-climate-change/transformational-change/scripts/cropped_areas"
...
####specify rcps
dset <- "cru"
cru_dir <- paste(met_dir,"/",dset,"_hist_yearly", sep="")
crop_dir <- paste(runs_dir,"/",dset,"_hist_yearly", sep="")
if (!file.exists(crop_dir)) {dir.create(crop_dir,recursive=T)}

#############  Projection onto future  ##################
#Run principal function for each GCM
cat("...calculate yearly suitability for historical period\n")

#loop aroudn GCMs
#end in 2098 because for 2099 next year does not exist so cannot simulate
for(ye in c(1986:2005)){
  #ye <- 1986
  #but then later he takes in aDir2 ye+1 so it acutally also uses 2099 potentially
  run_dir <- paste(crop_dir,"/",ye, sep="") #Output folder for each GCM
  tmet_dir <- paste(cru_dir, "/", ye, sep="")  #Folder of climate data for this year
  tmet_dir2 <- paste(cru_dir, "/", (ye+1), sep="")  #Folder of climate data for next year
  ...
  for (n in 1:ncrops) {
    ...
    if (!file.exists(paste(run_dir, "/", crop_name, "_suit.tif", sep=""))) {
      cat("...suitability sim for",crop_name,"/" , ye,"\n")
      ...
      if (crop_name == "sorghum") {
        ...
        his <- suitCalc(climPath = tmet_dir, 
                        climPath2 = tmet_dir2,
      ...
      cat("...suitability sim for",crop_name,"/", ye,"done\n")
    }
  }
}
