...
calcHal.matrix <- function (lag, params, training, weights, base, project)
{
 ...
 if (exists(string_c(".ccafshal_ext",paste(as.vector(params$to), collapse="")), env=globalenv())) {
    ll <- get(string_c(".ccafshal_ext",paste(as.vector(params$to), collapse="")), env=globalenv())
  } else {
    ll <- lapply(training, 
      function(x) extract(x,params$to))
    assign(string_c(".ccafshal_ext",paste(as.vector(params$to), collapse="")), weights_ext, env=globalenv())
  }
 ...
 # project - ref
 mrd <- lapply(1:length(params$vars), 
     function(x) ll[[which(params$idx_gcms==base & params$idx_vars == x)]][lag] - params$ref_train[[which(params$idx_gcms==base & params$idx_vars == x)]][lag])
 ...
 mrd <- lapply(1:length(params$vars), 
     function(x) mrd[[which(params$idx_gcms==base & params$idx_vars == x)]][lag] / ref_training_corrected[[which(params$idx_gcms==base & params$idx_vars == x)]][lag])
