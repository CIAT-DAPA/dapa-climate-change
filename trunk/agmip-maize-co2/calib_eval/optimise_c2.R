...
#1. halfway through ISTG=2 == DOY 200 [2007] OR DOY 207 [2008]
#2. TLIMJUV + 4 DAYS + 0.5 * TLIMSIL = 692.4 [2007]
#   TLIMJUV + 4 DAYS + 0.5 * TLIMSIL = 678.6 [2008]
#3. 0.5 * TLIMSIL + 0.90 * TLIMGFP = 594.6 [2007]
#   0.5 * TLIMSIL + 0.95 * TLIMGFP = 581.05 [2008]
...
bin_dir <- paste(wd,"/bin/glam-maize-c2",sep="")
    ...
    for (iter in 1:15) {
      ...
      for (i in 1:nrow(param_orig)) {
        ...
        for (j in 1:nsteps) {
          ...
          #1. halfway through STG=2 eq. 200 in 2007
          #   halfway through STG=2 eq. 207 in 2007
          ...
          flwr_doy_2007 <- flwr_per_2007$DOY[round(nrow(flwr_per_2007)/2)]
          flwr_doy_2008 <- flwr_per_2008$DOY[round(nrow(flwr_per_2008)/2)]
          ...
          tthalf_2007 <- pflwr_s0 + pflwr_s1 + 0.5 * pflwr_s2
          ...
          pflwr_s1 <- pflwr[which(pflwr$STG == 1),]; if (nrow(pflwr_s1) == 0) {pflwr_s1 <- 0} else {pflwr_s1 <- cumsum(pflwr_s1$TT)[nrow(pflwr_s1)]}
          pflwr_s2 <- pflwr[which(pflwr$STG == 2),]; if (nrow(pflwr_s2) == 0) {pflwr_s2 <- 0} else {pflwr_s2 <- cumsum(pflwr_s2$TT)[nrow(pflwr_s2)]}
          tthalf_2008 <- pflwr_s0 + pflwr_s1 + 0.5 * pflwr_s2
          ...
          ttmat_2007 <- 0.5 * pmat_s2 + pmat_s3 + 0.9 * (pmat_s4 + pmat_s5)
          ...
          pmat_s4 <- pmat[which(pmat$STG == 4),]; if (nrow(pmat_s4) == 0) {pmat_s4 <- 0} else {pmat_s4 <- cumsum(pmat_s4$TT)[nrow(pmat_s4)]}
          pmat_s5 <- pmat[which(pmat$STG == 5),]; if (nrow(pmat_s5) == 0) {pmat_s5 <- 0} else {pmat_s5 <- cumsum(pmat_s5$TT)[nrow(pmat_s5)]}
          ttmat_2008 <- 0.5 * pmat_s2 + pmat_s3 + 0.95 * (pmat_s4 + pmat_s5)
