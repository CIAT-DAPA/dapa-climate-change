...
theEntireProcess <- function(spID, OSys, inputDir) {
	...
	if (!file.exists(verFile)) {
		...
		inCurClimDir <- paste(inputDir, "/raw_climate_data/Baseline", sep="")
		
		cat("Taxon ", spID, "\n")
	  
		#1. Load species data
	  
		occFile <- paste(inputDir, "/occurrence_files/", spID, "_swd.csv", sep="")
		...
		if (file.exists(occFile)) {
			...
			if (nOcc > 0) {
				...
				if (!file.exists(outName)) {
					...
					dir.create(paste(outName, "/projections/clamping", sep=""))
					dir.create(paste(outName, "/projections/averages", sep=""))
					dir.create(paste(outName, "/projections/summarize", sep=""))
				}
				
				cat("Crossvalidating the model... \n")
				# cat(paste("java", "-mx512m", "-jar", maxentApp, "-s", outFileName, "-e", backFileSwd, "-o", paste(outName, "/crossval", sep=""), "-P", "replicates=25", "nowarnings", "-a", "-z"))
				...
				cat(paste("java", "-mx512m", "-jar", maxentApp, "-s", outFileName, "-e", backFileSwd, "-o", paste(outName, "/crossval", sep=""), "-P", "replicates=25", "replicatetype=crossvalidate", "nowarnings", "-a", "-z", "-j", inCurClimDir))
				system(paste("java", "-mx512m", "-jar", maxentApp, "-s", outFileName, "-e", backFileSwd, "-o", paste(outName, "/crossval", sep=""), "-P", "replicates=25", "replicatetype=crossvalidate", "nowarnings", "-a", "-z", "-j", inCurClimDir), wait=TRUE)
				...
				if (procSwitch) {
					...
					for (prj in projectionList) {
						...
						distMean <- writeRaster(distMean, paste(outName, "/projections/averages/", spID, "_", suffix, "_EMN.asc", sep=""), format="ascii", overwrite=T)
						cat("Calculating and writing std \n")
						distStdv <- calc(stack(otList), fun)
						distStdv <- writeRaster(distStdv, paste(outName, "/projections/averages/", spID, "_", suffix, "_ESD.asc", sep=""), format="ascii", overwrite=T)
						...
						distMeanPA <- writeRaster(distMeanPA, paste(outName, "/projections/averages/", spID, "_", suffix, "_EMN_PA.asc", sep=""), format='ascii', overwrite=T)
						distMeanPR <- writeRaster(distMeanPR, paste(outName, "/projections/averages/", spID, "_", suffix, "_EMN_PR.asc", sep=""), format='ascii', overwrite=T)
						distStdvPR <- writeRaster(distStdvPR, paste(outName, "/projections/averages/", spID, "_", suffix, "_ESD_PR.asc", sep=""), format='ascii', overwrite=T)
					...
					# # ftoZIP <- list.files(paste(outName, "/projections/clamping", sep=""), pattern=".asc")
					# # cat("Compressing... \n")
					# # for (fz in ftoZIP) {
						# # fName <- paste(outName, "/projections/clamping", fz, sep="")
						# # if (OSys == "linux") {
