...
#This is to avoid constant loading of individual raster data.
...
#extract soil data
xy_main <- cbind(xy_main, as.data.frame(extract(rll, xy_main[,c("x","y")]))*0.01)
xy_main <- cbind(xy_main, as.data.frame(extract(dul, xy_main[,c("x","y")]))*0.01)
xy_main <- cbind(xy_main, as.data.frame(extract(sat, xy_main[,c("x","y")]))*0.01)
xy_main <- cbind(xy_main, as.data.frame(extract(bud, xy_main[,c("x","y")]))*0.01)
...
xy_main$SLDR <- apply(xy_main[,paste("DRATE_",1:8,sep="")],1,FUN=function(x) {y<-mean(x,na.rm=T); return(y)})
...
save(list=c("xy_main"),file=paste(mdataDir,"/initial_conditions_major_dssat.RData",sep=""))
...
#save crop yields
save(list=c("xy_main_yield"),file=paste(mdataDir,"/yield_major_dssat.RData",sep=""))
