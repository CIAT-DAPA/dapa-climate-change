...
NPROC=$(qstat -u earjr | grep run_csixm | wc -l)
NPROCMAX=100
...
do
	...
	do
		...
		if [ ! -f run_csixm.sh ]
		then
			cp -vf ~/Repositories/dapa-climate-change/trunk/robustness/ppe/dssat_hypercube_run.sh run_csixm.sh
			chmod 755 run_csixm.sh
		fi
		
		#create out folder if it does not exist
		if [ ! -d ~/quest-for-robustness/scratch/procfiles/dssat_${CSMODEL}_out_${ME}_${IRUN} ]
		then
			$(mkdir ~/quest-for-robustness/scratch/procfiles/dssat_${CSMODEL}_out_${ME}_${IRUN})
		fi
		
		#do the model run only if procfile does not exist
		if [ ! -f ~/quest-for-robustness/scratch/procfiles/dssat_${CSMODEL}_out_${ME}_${IRUN}/dssat_${CSMODEL}_out_${ME}_${IRUN}_${ILOC}.proc ]
		then
			echo ----------------------------------------------------------
			echo ---- submitting ${TPID} -----
			echo ---- ${NPROC} processess queueing currently -----------
			echo ----------------------------------------------------------
			qsub run_csixm.sh $ME $IRUN $ILOC $CSMODEL
		else
			echo ----------------------------------------------------------
	...
	NPROCFIL=$(ls -l ~/quest-for-robustness/scratch/procfiles/dssat_${CSMODEL}_out_${ME}_${IRUN}/dssat_${CSMODEL}_out_${ME}_${IRUN}_*.proc | grep .proc | wc -l)
	if [ ${NPROCFIL} -eq ${MAXILOC} ]
	then
		#remove runfiles
		rm -f ~/quest-for-robustness/scratch/runfiles/run_csixm.sh.e*
		rm -f ~/quest-for-robustness/scratch/runfiles/run_csixm.sh.o*
	else
