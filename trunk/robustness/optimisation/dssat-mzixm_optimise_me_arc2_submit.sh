  ...
  for INUM in $(seq 1 $MAXINUM)
	do
		...
		do
			...
			if [ ! -f run_csixm.sh ]
			then
				cp -vf ~/Repositories/dapa-climate-change/trunk/robustness/optimisation/dssat-csixm_optimise_me_arc2_run.sh run_csixm.sh
			...
			if [ ! -d ~/quest-for-robustness/scratch/procfiles/dssat_${CSMODEL}_out_${ME}_${ITER} ]
  			...
  			then
				...
				echo ----------------------------------------------------------
	  			qsub run_csixm.sh $ME $ITER $INUM $JNUM
	  		else
  				echo ----------------------------------------------------------
