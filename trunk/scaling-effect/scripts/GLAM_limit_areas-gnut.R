...
pdf(paste(figDir,"/gnut_yield_monfreda.pdf",sep=""), height=4,width=5,pointsize=12,family="Helvetica")
...
tplot <- tplot + layer(panel.text((g2@xmin+g2@xmax)*.5, (g2@ymin+g2@ymax)*.5, "G",cex=1.25))
print(tplot)
dev.off()
...
pdf(paste(figDir,"/gnut_yield_glam.pdf",sep=""), height=4,width=5,pointsize=12,family="Helvetica")
...
pdf(paste(figDir,"/gnut_yield_glam_ygp050.pdf",sep=""), height=4,width=5,pointsize=12,family="Helvetica")
tplot <- rs_levplot2(gyield,zn=NA,zx=NA,nb=NA,brks=seq(0,4.5,by=0.25),scale="Spectral",col_i="red",col_f="#FEE0D2",ncol=11,rev=F,leg=T)
tplot <- tplot + layer(sp.polygons(as(g2,'SpatialPolygons'),lwd=1.25,col="blue"))
tplot <- tplot + layer(panel.text((g2@xmin+g2@xmax)*.5, (g2@ymin+g2@ymax)*.5, "G",cex=1.25))
print(tplot)
