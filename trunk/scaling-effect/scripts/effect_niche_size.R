...
pg_th1_n <- length(which(xy$aharv >= 0.1 & xy$suit >= th1)) / length(which(xy$aharv >= 0.1)) * 100
pg_th2_n <- length(which(xy$aharv >= 0.1 & xy$suit >= th2)) / length(which(xy$aharv >= 0.1)) * 100
...
for (i in 1:nrow(sensruns)) {
  ...
  tpg_th1_n <- length(which(xy$aharv >= 0.1 & xy$tsuit >= th1)) / length(which(xy$aharv >= 0.1)) * 100
  tpg_th2_n <- length(which(xy$aharv >= 0.1 & xy$tsuit >= th2)) / length(which(xy$aharv >= 0.1)) * 100
...
#### line plots
#plot change in niche size vs. temperature
...
#plot change in niche size vs. precipitation
...
pdf(paste(figDir,"/extra_plots/niche_size_change_temperature_maize_boxplot.pdf",sep=""), height=6,width=8,pointsize=16)
par(mar=c(5,5,1,1),las=1,lwd=1.75)
...
pdf(paste(figDir,"/extra_plots/niche_size_change_precipitation_maize_boxplot.pdf",sep=""), height=6,width=8,pointsize=16)
par(mar=c(5,5,1,1),las=1,lwd=1.75)
