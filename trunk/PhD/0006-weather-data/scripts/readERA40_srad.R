...
oDir <- "./daily_data_srad"
if (!file.exists(oDir)) {dir.create(oDir)}

varName <- "Surface net solar radiation Surface"
...
system.time(sfSapply(as.vector(iniYear:finYear),year_wrapper_srad))

sfStop()


# #read some data
# stk <- stack(paste(oDir,"/1985/srad_",1:365,".asc",sep=""))
# vs <- extract(stk,cbind(X=40,Y=5))
