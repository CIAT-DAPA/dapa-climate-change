...
if (!file.exists(paste(cd,"/data/detrended-IND2-",cropName,"-summary.csv",sep=""))) {
  sData <- calcSummary(yieldData,"DISID",yfds,hfds,pfds,tser,cd,cropName,
...
#clean mask raster
if (!file.exists(paste(cd,"/0_base_grids/india-1min-msk.asc",sep=""))) {
  rs <- createMask(shp,resol)
  rs <- writeRaster(rs,paste(cd,"/0_base_grids/india-1min-msk.asc",sep=""),format='ascii')
} else {
  rs <- raster(paste(cd,"/0_base_grids/india-1min-msk.asc",sep=""))
}
