...
syDir <- paste(ecoDir,"/suit_yield2",sep="") #suit_yield2 is for actual yield. suit_yield is for tprod
...
plot(ynor_all$EXP.MEAN,sbar_all$RUN.MEAN,xlim=c(0,1),ylim=c(0,100),pch=20,
     xlab="GLAM yield (normalised)",ylab="Suitability (%)")
grid()
abline(a=0,b=100)
dev.off()
...
bp <- boxplot(all_gem$value ~ all_gem$CLASS,pch=NA,ylim=c(0,100),las=2,col="grey",
              ylab="Mean EcoCrop suitability",xlab="GLAM yield quantile (normalised)")
grid()
medvals <- data.frame(class=1:ncol(bp$stats),value=bp$stats[3,])
