...
splineFitting <- function(anuDir="C:/anu/Anuspl43/bin", stDir, rDir, oDir, nfolds=100, train.per=0.85, vn="rain", unix=F) {
  ...
  for (fold in 1:nfolds) {
    ...
    train <- sample(1:nrow(st.sel.10y), nrow(st.sel.10y)*train.per)
    st.sel.10y$TRAIN_TEST <- NA; st.sel.10y$TRAIN_TEST[train] <- "TRAIN"
    st.sel.10y$TRAIN_TEST[-train] <- "TEST"
