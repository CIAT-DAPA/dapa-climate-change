...
compareMeanCellGHCN <- function(wd="",gcm="",variable="rain",divide=F,iso="ETH",isoDir="") {
  #General stuff
  ...
  if (!file.exists(outMetFile)) {
    ...
    in.data <- read.csv(paste(inpDir,"/",dataset,"_",variable,"_1961_1990_mean.csv",sep=""))
    locations <- data.frame(ID=in.data$ID,LAT=in.data$LAT,LONG=in.data$LONG)
