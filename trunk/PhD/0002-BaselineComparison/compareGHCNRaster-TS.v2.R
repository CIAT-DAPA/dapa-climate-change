...
gcmmd="bccr_bcm2_0", iso="ETH", timeseries=c(1961:1990)) {
  ...
  if (!file.exists(paste(odmGCM, "/metrics-", iso, ".csv", sep=""))) {
    ...
    st.uniqueXY <- xyFromCell(msk,st.unique)
    rm(msk.cellNumbers); rm(st.cellNumbers); g=gc()
    
    if (length(st.ids) > 0) {
      cat("Analysing", length(st.ids), "stations \n")
      cat("Resulted in", length(st.uniqueXY), "locations \n")
      
      #Extract the timeseries for these unique locations
      un.counter <- 1
      for (un.pt in nrow(st.uniqueXY)) {
        cat("Processing location", paste(st.unique[un.pt]), "\n")
...
#variable (vg)
#model name (gcm) in
#a hard drive location (yrdir)
#vector of years (yr.list)

extractTS <- function(xy, yrdir, yr.list, mth, vg="prec", gcm="bccr_bcm2_0") {
...
#st.data should contain LAT,LON,MONTHLY VALUES
#GCM data should be YEAR,mth.name.GCM (e.g. 1961,JAN.GCM)
