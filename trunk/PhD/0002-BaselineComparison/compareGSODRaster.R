...
st.extractGCMCell <- function(x, stations, mnth="JAN", gcmRes) {
  ...
  if (length(comp.data) == 0) {res <- c(NA,NA,NA)} else {res <- c(mean(comp.data,na.rm=T),max(comp.data,na.rm=T),min(comp.data,na.rm=T))}
  return(res)
}
...
compareGSODR <- function(gcmDir=NULL, gcm=NULL, shpDir=NULL, stationDir=NULL, country=NULL, variable=NULL, divide=T, months=c(6,7,8), outDir=NULL, verbose=T) {
  ...
  if (!file.exists(paste(outDir, "/", lgname, sep=""))) {
    ...
    #Loading stations
    st <- read.csv(paste(stationDir, "/gsod-", st.varName, "-1961_1990-mean.csv", sep=""))
