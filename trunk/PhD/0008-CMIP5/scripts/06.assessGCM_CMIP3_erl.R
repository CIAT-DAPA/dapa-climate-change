...
inputDD <- paste(cmip5Dir,"/assessment/input-data",sep="")
outputDD <- paste(cmip5Dir,"/assessment/output-data-cmip3",sep="")
if (!file.exists(outputDD)) {dir.create(outputDD)}
...
######################################################################
######################################################################
#These are all preliminary calculations. So they are commented out
#Uncomment if these need to be run again
######################################################################
######################################################################
######################################################################
...
# for (year in yri:yrf) {
#   #year <- yri
#   oyrDir <- paste(moDir,"/",year,sep="")
#   if (!file.exists(oyrDir)) {dir.create(oyrDir)}
#   
#   #loop through variables and months
#   for (vn in c("prec","tmean","tmin","tmax","dtr")) {
#     #vn <- "prec"
#     cat("year:",year,": var :",vn,":")
#     for (m in 1:12) {
#       #m <- 1
#       cat(m,". ",sep="")
#       mth <- sprintf("%02d",m)
#       fList <- paste(mdDir,"/",gcmList,"/yearly_files/",year,"/",vn,"_",mth,".nc",sep="")
#       fList <- as.character(sapply(fList,FUN=checkExists))
#       fList <- fList[which(!is.na(fList))]
#       
...
#       rstk <- stack(mList_res)
#       rs <- calc(rstk,fun=function(x) {mean(x,na.rm=T)})
#       rs <- writeRaster(rs,paste(moDir,"/",vn,"_",mth,".nc",sep=""),
#                         format="CDF",overwrite=T,varname=vn,xname="lon",yname="lat",zname="time")
#       rm(list=c("rstk","rs","mList","mList_res")); g=gc(); rm(g)
#     }
#   }
