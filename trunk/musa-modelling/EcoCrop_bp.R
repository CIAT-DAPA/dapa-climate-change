...
#First the workspace has to be loaded
require(rgdal)
require(raster)
...
suitCalc <- function(climPath='', Gmin=90,Gmax=90,Tkmp=0,Tmin=10,Topmin=16,Topmax=25,Tmax=35,Rmin=150,Ropmin=300,Ropmax=400,Rmax=600, outfolder='', cropname='',ext=".asc") {
  ...
  createLog <- makeLogFile(filePathName=logFileName, climPath, cropname, Gmin, Gmax, Tkmp, Tmin, Topmin, Topmax, Tmax, Rmin, Ropmin, Ropmax, Rmax)
  ...
  #this can be plot as results plot(fsuitRaster)
  ...
  bs <- blockSize(climateStack, minblocks=2)
  cat("(", bs$n, " chunks) \n", sep="")
