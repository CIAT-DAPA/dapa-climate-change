...
if len(sys.argv) < 3:
	os.system('cls')
	print "\n Too few args"
	print "   - ie: python 00-split-monthly-raw-data.py T:\data\gcm\cmip5\raw\monthly rcp26"
	sys.exit(1)
...
varList = {"tas": "tmean", "tasmax": "tmax", "tasmin": "tmin", "pr": "prec"}
		...
		if os.path.isdir(rcpDir + "\\" + model + "\\" + ens):
			...
			if not os.path.exists(checkFile):
				...
				for var in varList:
					...
					merList = sorted(glob.glob(ensDir + "\\" + varList[var] + "*.nc*"))
					for merNc in merList:
