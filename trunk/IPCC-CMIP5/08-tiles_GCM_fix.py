...
if rcp=='rcp26':
	...
	modellist = ['bcc_csm1_1', 'bcc_csm1_1_m', 'bnu_esm', 'cccma_canesm2', 'cesm1_bgc', 'cesm1_cam5', 'csiro_access1_0', 'csiro_access1_3', 'csiro_mk3_6_0', 'fio_esm', 'gfdl_cm3', 'gfdl_esm2g', 'gfdl_esm2m', 'giss_e2_h_cc', 'giss_e2_r', 'giss_e2_r_cc', 'inm_cm4', 'ipsl_cm5a_lr', 'ipsl_cm5a_mr', 'lasg_fgoals_g2', 'miroc_esm', 'miroc_esm_chem', 'miroc_miroc5', 'mohc_hadgem2_cc', 'mohc_hadgem2_es', 'mpi_esm_lr', 'mri_cgcm3', 'ncar_ccsm4', 'ncc_noresm1_m', 'nimr_hadgem2_ao']	
	# modellist = ['inm_cm4']
elif rcp=='rcp60':
	modellist = ['bcc_csm1_1', 'bcc_csm1_1_m', 'cesm1_cam5', 'csiro_mk3_6_0', 'fio_esm', 'gfdl_cm3', 'gfdl_esm2g', 'gfdl_esm2m', 'giss_e2_h', 'giss_e2_r', 'ipsl_cm5a_lr', 'miroc_esm', 'miroc_esm_chem', 'miroc_miroc5', 'mohc_hadgem2_es', 'mri_cgcm3', 'ncar_ccsm4', 'ncc_noresm1_m', 'nimr_hadgem2_ao']	
else:
			...
			for lat in sorted(latDc):
				for lon in sorted(lonDc): 
					...
					if os.path.exists(inZip):
						if var=="cons":
							sizeTile=os.path.getsize(inZip)/float(1000000) < 0.300
						else:
							sizeTile=os.path.getsize(inZip)/float(1000000) < 5
					
					if not os.path.exists(inZip):
						# print inZip
						check.append(model + "-" + rcpDc[rcp] + "-" + period + "-" + var+ "-" + lat + lon)
					elif sizeTile:
						check.append(model + "-" + rcpDc[rcp] + "-" + period + "-" + var+ "-" + lat + lon)
...
for item in  list(set(check)):
	...
	if not os.path.exists(checkfile):	
		...
		print "\n\tprocessing", model,period,var,zoneTile + "\n"
		...
		for raster in rasterList:
			if not arcpy.Exists(dirgrids+"\\"+raster+"_17"):
				...
				print "\t ", os.path.basename(raster), " exists!"		
		...
		for raster in rasterList:
			...
			if zoneDc[zone]==zoneTile:
				print "Convert to ESRI-Ascii", zoneTile			
		...
		######## else:
		inZip = dirProc + "\\ZipVarZonas\\" + model + "_" + rcpDc[rcp] + "_" + period + "_" + var + "_" + res + "_" + ens + "_" + zoneTile + "_asc.zip"
		if not os.path.exists(inZip):
		...
		os.system("rmdir /s /q " + dirzip)
		os.system("rmdir /s /q " + dirascii)
