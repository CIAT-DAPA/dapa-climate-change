...
if len(sys.argv) < 5:
	os.system('cls')
	print "\n Too few args"
	print "   - ie: python check_files_global.py S:\data\portals\analogues-gcm-data\TilesByCountry a1b 2_5min 2020_2049"
	sys.exit(1)
...
countrytilelist = "arm", "chn", "cri", "cyp", "cze", "grl", "idn", "ind", "irn", "ita", "jpn", "kaz", "lao", "lby", "mar", "mdg", "pak", "per", "pol", "prk", "prt", "pry", "rom", "rwa", "sau", "sdn", "sjm", "slb", "som", "tha", "tkm", "ton", "tur", "tza", "ukr", "vct", "zaf"
...
if sres == "baseline":
	...
	for country in countrytilelist:
		...
		for var in monthDic:
			...
			tifdir = dirbase + "\\" + sres + "\\" + "global\\"	+ "_" + resolution + "\\" + model + "\\" + period + "\\" + var + "_tif"
	...
	for model in modellist:
		...
		for var in monthDic:
			...
			print "\n --> Check files: " + "global_" + resolution + sres,model,period,var,"\n"
			
		
			tifdir = dirbase + "\\sres_" + sres + "\\" + "global"	+ "_" + resolution + "\\" + model + "\\" + period + "\\" + var + "_tif"
			

			
			# for tile in range(0, 161 + 1, 1):
			...
			for month in range(1, int(monthDic [var]) + 1, 1):
				...
				for tile in range(0, 161 + 1, 1):
					...
					if var + "_tif" == 'bio_tif':
						...
						tiff = tifdir + "\\" + sres + "_" + period + "_" + model + "_" + var + "_" + str(month) + "_" + str(tile) +"_1" + ".tif"
					else:
						tiff = tifdir + "\\" + sres + "_" + period + "_" + model + "_" + var + "_" + str(month) + "_" + str(tile)  + ".tif"
					
					if not os.path.exists(tiff):
						print model,os.path.basename(tiff),"NO"
						outFile = open(dirbase + "\\" + sres + "_check_global.txt", "a")
						outFile.write("global_" + resolution + "\t" + model + "\t" + tiff + "\n")
						outFile.close()
						
					else:
						print model,os.path.basename(tiff),"OK"
					
	print "\n","global_" + resolution ," checked!"
