...
id_rhum <- read.csv(paste(bDir, "/rhum_metrics.csv", sep=""))
...
tiff(paste0(oDir, '/skill_interpolation_metrics_llanos.tif'), width=1800, height=900, pointsize=8, compression='lzw',res=150)
...
rsq_rhum_max <- max(id_rhum$R2.TEST)

# tiff('fig-3d-rsq_tmean_monthly.tif',width=600, height=600,pointsize=8,compression='lzw',res=150)
par(mar=c(4,4,2,1))
boxplot(id_rhum$R2.TEST ~ id_rhum$MONTH,xlab='Mes',ylab='R2',pch=20,cex=1.1,ylim=c(0,1), names=(mth_lb), cex.axis=0.9, col="gray90")
# text(x=11.9, y=0.98,"D", pos=4, cex=1.1.4)
title("HR", line = 1)
grid()
lines(c(0,13),c(1 *.5, 1 *.5),col='red',lty=2)
...
legend("bottomleft", c("G"), box.lty=0, bg="transparent", cex = 2, yjust= 0.5, inset=c(-0.03,0))
# dev.off()


rmse_rhum_max <- max(id_rhum$RMSE.TEST)

# tiff('fig-3f-rmse_tmean_monthly.tif',width=600, height=600,pointsize=8,compression='lzw',res=150)
par(mar=c(4,4,2,1))
boxplot(id_rhum$RMSE.TEST ~ id_rhum$MONTH,xlab='Mes',ylab='ECM (%)',pch=20,cex=1.1,ylim=c(0,rmse_rhum_max), names=(mth_lb), cex.axis=0.9, col="gray90")
# text(x=11.9, y=0.98,"F", pos=4, cex=1.1.4)
grid()
lines(c(0,13),c(rmse_rhum_max *.5, rmse_rhum_max *.5),col='red',lty=2)
lines(c(0,13),c(rmse_rhum_max *.75,rmse_rhum_max *.75),col='red')
legend("bottomleft", c("H"), box.lty=0, bg="transparent", cex = 2, yjust= 0.5, inset=c(-0.03,0))
# dev.off()
