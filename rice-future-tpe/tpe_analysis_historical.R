...
#############################################################################################
### for each cluster create the water stress profile graph
#############################################################################################

#list of clusters
clus_list <- unique(rs_data$Cluster)

#note: cluster 1: LFE; cluster 2: HFE; cluster 3: FE
#taken from boxplot(op_data$WRR14 ~ op_data$Cluster, pch=20)

#make legend / colour data.frame
leg_df <- data.frame(env_cluster=c(1,1,1,2,2,3,3,3),
                     stress_profile=c(1,2,3,1,2,1,2,3),
                     env_leg=c("LFE","LFE","LFE","HFE","HFE","FE","FE","FE"),
                     col_prof=c("blue","dark green","red","blue","dark green","blue","dark green","red"))

#frequencies of stress profiles
stress_freq <- as.data.frame(as.matrix(table(st_data[,c("Env","clust")])))

#loop clusters
stress_df <- data.frame()
for (clus in clus_list) {
  #clus <- clus_list[1]
  clus_data <- rs_data[which(rs_data$Cluster == clus),]
  clus_data <- clus_data[which(!is.na(clus_data$stress_profile)),]
  stress_list <- unique(clus_data$stress_profile)
  
  #frequency table
  clus_freq <- stress_freq[which(stress_freq$Env == clus),]
  clus_freq$FRel <- round(clus_freq$Freq / sum(clus_freq$Freq, na.rm=T) * 100, 1)
  clus_freq <- merge(clus_freq, leg_df, by.x=c("Env","clust"), by.y=c("env_cluster","stress_profile"))
  clus_freq$leg <- paste("SP [",clus_freq$clust,"] = ",clus_freq$FRel," %", sep="")
  
  for (stp in stress_list) {
    #stp <- stress_list[1]
    stress_data <- clus_data[which(clus_data$stress_profile == stp),]
    stress_data <- aggregate(stress_data[,c("PCEW","DVS")], by=list(index=stress_data$index),FUN=function(x) {mean(x,na.rm=T)})
    stress_data <- stress_data[stress_data$index %in% c("C","D","E","F","G","H","I","J","K","L","M"),]
    stress_data <- stress_data[order(stress_data$index),]
    stress_data$dae <- seq(21,84,by=7)
    row.names(stress_data) <- NULL
    ...
    #colour
    tcol <- paste(leg_df$col_prof[which(leg_df$env_cluster == clus & leg_df$stress_profile == stp)])
    
    if (stp == stress_list[1]) {
      par(mar=c(5,5,1,1), lwd=1.25)
      plot(stress_data$dae, stress_data$PCEW, ty='l', lwd=2, col=tcol, ylim=c(0.2,1),
           xlab="Days after emergence", ylab="Stress index")
    } else {
      lines(stress_data$dae, stress_data$PCEW, lwd=2, col=tcol)
    }
  }
  
  #add legend
  grid()
  legend("bottomleft", lty=1, lwd=2, legend=paste(clus_freq$leg), col=paste(clus_freq$col_prof), 
         text.col=paste(clus_freq$col_prof), title="Frequency", title.col="black")
