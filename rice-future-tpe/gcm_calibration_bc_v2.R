...
gcm_extraction <- function(var="tasmax", rcp="historical", ts="1950_2000", gcmlist=c("bcc_csm1_1", "bcc_csm1_1_m", "bnu_esm", "cccma_cancm4", "cccma_canesm2"),lon=-73.5, lat=3.4, dirgcm="T:/gcm/cmip5/raw/daily", dirout="D:/workspace/bc"){
    ...
    if (rcp == "historical"){
      ...
      for (j in 1:ngcm) {
        ...
        if (varmod == "prec" || varmod == "srad"){ 
          bc_values[,j] <- odat[,j+2] * ( ( avgobs_m - avggcm_l[[j]] ) / max(c(avggcm_l[[j]],0.001)) + 1 )
      ...
      for (j in 1:ngcm) {
        ...
        if (varmod == "prec" || varmod == "srad"){ 
          bc_values[,j] <- odat_f[,j+2] * ( ( avgobs_m_f - avggcm_l_f[[j]] ) / max(c(avggcm_l_f[[j]],0.001)) + 1 )
          bc_values[bc_values<0] <- 0
        } else {
    ...
    if (rcp == "historical"){
      ...
      for (j in 1:ngcm) {
        ...
        if (varmod == "prec" || varmod == "srad"){
          ...
          bc_values[,j] <- avgobs_m *  (1 + ( stdobs_m / stdgcm_l[[j]] * ( odat[,j+2] - avggcm_l[[j]]) / max(c(avgobs_m,0.001)) ) )  ## JRV
          bc_values[bc_values<0] <- 0
        } else {
      ...
      for (j in 1:ngcm) {
        ...
        if (varmod == "prec" || varmod == "srad"){
          ...
          bc_values[,j] <- avgobs_m_f *  (1 + ( stdobs_m_f / stdgcm_l_f[[j]] * ( odat_f[,j+2] - avggcm_l_f[[j]]) / max(c(avgobs_m_f)) ) )  ## JRV
          bc_values[bc_values<0] <- 0
        } else {
...
del_calcs <- function(varmod="prec", rcp="rcp45", lon=-73.5, lat=3.4, dirbase="D:/CIAT/Workspace/bc"){
  ...
  if (!file.exists(bcdat)) {
    ...
    for (j in 1:ngcm) {
      ...
      if (varmod == "prec" || varmod == "srad"){ 
        bc_values[,j] <- odat[,2] * ( (avggcm_l_f[[j]] - avggcm_l[[j]]) / max(c(avggcm_l[[j]],0.001)) + 1 )
        bc_values[bc_values<0] <- 0
      } else {
...
cf_calcs <- function(varmod="tmax", rcp="rcp45", lon=-73.5, lat=3.4, dirbase="D:/CIAT/Workspace/bc"){
  ...
  if (!file.exists(bcdat)) {
    ...
    for (j in 1:ngcm) {
      ...
      if (varmod == "prec" || varmod == "srad"){
        #bc_values[,j] <- odat[,2] * (1 + ( stdgcm_l_f[[j]] / stdgcm_l[[j]]  * ( avggcm_l_f[[j]] - avggcm_l[[j]] ) / avggcm_l[[j]] ) )
        #bc_values[,j] <-  avggcm_l_f[[j]] * (1 + ( stdgcm_l_f[[j]] / stdgcm_l[[j]]  * ( odat[,2] - avggcm_l[[j]] ) / max(c(avggcm_l_f[[j]],0.001)) ) ) #JRV
        bc_values[,j] <- odat[,2] * (1 + ( stdgcm_l_f[[j]] / stdgcm_l[[j]]  * ( avggcm_l_f[[j]] - avggcm_l[[j]] ) / max(c(avggcm_l[[j]],0.001)) ) ) #JRV-v2
        bc_values[bc_values<0] <- 0
      }else{
        bc_values[,j] <- avggcm_l_f[[j]] + ( stdgcm_l_f[[j]] / stdgcm_l[[j]] * (odat[,2] - avggcm_l[[j]]) )        
      }
...
bc_stats <- function(varmod="prec", rcp="historical", ts="1950_2000", lon=-73.5, lat=3.4, dirbase="D:/CIAT/Workspace/bc"){
  ...
  if (rcp == "historical"){
    ...
    methods <- c("cf", "del", "raw") #, "bc", "sh", "qm", "raw")
    methods_ln <- c("CF Var", "CF", "raw") #, "BC Var", "BC", "QM", "RAW")
  }
  ...
  if (varmod == "srad"){ylabel <- "Shortwave Sol. Radiation (W/m2)"; limit = c(0, 400)}
  ...
  for (i in 1:ngcm){
    ...
    ots <- paste0(dirout, "/ts_", rcp,"_",gcm,"_",varmod,"_lon_",lon,"_lat_",lat,".pdf")
    ...
    if (!file.exists(ots)) {
      ...
      if (rcp == "historical"){  # Historical plot includes observations
        ...
        pdf(ots, width=5, height=6, pointsize=8)
  ...
  ots <- paste0(dirout, "/spread_", rcp,"_",varmod,"_lon_",lon,"_lat_",lat,".pdf")
  ...
  if (!file.exists(ots)){
    ...
    if (rcp == "historical"){  # Historical plot includes observations
      ...
      #tiff(ots, width=1200, height=1000, pointsize=8, compression='lzw',res=100)
      pdf(ots, width=5, height=7, pointsize=8)
      p <- ggplot() + 
  ...
  if (!file.exists(intannvar)) {
    ...
    if (rcp == "historical"){   # Historical plot includes observations
      ...
      for (i in 1:ngcm){
        ...
        pdf(paste0(dirout, "/var_", rcp,"_",gcm,"_",varmod,"_lon_",lon,"_lat_",lat,".pdf"), width=5, height=7, pointsize=8)
    ...
    if (!file.exists(ofreq)) {
      ...
      if (rcp == "historical"){ # Historical plot includes observations
        ...
        for (i in 1:ngcm){
          ...
          pdf(paste0(dirout, "/freq_", rcp,"_",gcmlist[i],"_",varmod,"_lon_",lon,"_lat_",lat,".pdf"), width=7, height=4, pointsize=8)
