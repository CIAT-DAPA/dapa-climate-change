...
if len(sys.argv) < 4:
	os.system('cls')
	print "\n Too few args"
	print "   - ie: python Describe_Downscaled.py M:\climate_change\IPCC_CMIP3\ A1B F:\IPCC_CMIP3_process miub_echo_g"
	sys.exit(1)
...
if os.path.isfile(dirout + "\\Downscaled_SRES_" + scenario + "_" + model + ".txt"):
    outFile = open(dirout + "\\Downscaled_SRES_" + scenario + "_" + model + ".txt", "a")
else:
    outFile = open(dirout + "\\Downscaled_SRES_" + scenario + "_" + model + ".txt", "w")
