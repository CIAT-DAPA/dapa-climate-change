...
if len(sys.argv) < 3:
	...
	print "   - ie: python Ascii2Gridi.py D:\climate_change\Tyndall_Data\Global_30s\ukmo_hadgem1\2070s\_asciis Y:\Tyndall_data\A1Bp50\30yrAverages"
	sys.exit(1)
...
for asc in asclist:
	...
	OutRaster10 = dirout + "\\Global_10min\\ukmo_hadgem1\\2070s\\" + os.path.basename(asc)[:-4] 
	if not gp.Exists(OutRaster10):
		print OutRaster10
		gp.Resample_management(OutRaster, OutRaster10, "0.166667", "NEAREST")	
		
	OutRaster5 = dirout + "\\Global_5min\\ukmo_hadgem1\\2070s\\" + os.path.basename(asc)[:-4] 
	if not gp.Exists(OutRaster5):
		gp.Resample_management(OutRaster, OutRaster5, "0.08333333", "NEAREST")	

	OutRaster2_5 = dirout + "\\Global_2_5min\\ukmo_hadgem1\\2070s\\" + os.path.basename(asc)[:-4] 
