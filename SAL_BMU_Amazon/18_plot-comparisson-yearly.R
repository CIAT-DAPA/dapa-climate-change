...
oDir <- "D:/cenavarro/lat-sal/outputs/performance"
years <- c("1981_2010")
...
for (yr in years){
  ...
  for (var in varList){
    ...
    tiff(paste(oDir, "/plot_mths_", var, ".tif", sep=""), width=1000, height=1400, pointsize=8, compression='lzw',res=100)
    
    print(levelplot(plot, at = zvalues, scales = list(draw=FALSE),  xlab="", ylab="", par.settings = myTheme, colorkey = list(space = "bottom"))) # + layer(sp.polygons(mask, col="grey")) # + layer(sp.polygons(geotopo)))
...
for (yr in years){
  ...
  for (var in varList){
    ...
    tiff(paste(oDir, "/plot_seasons_", var, ".tif", sep=""), width=1200, height=800, pointsize=8, compression='lzw',res=100)
    
    print(levelplot(plot, at = zvalues, scales = list(draw=FALSE), layout=c(4, 1), xlab="", ylab="", par.settings = myTheme, colorkey = list(space = "bottom"))) # + layer(sp.polygons(mask), col="grey")) # + layer(sp.polygons(geotopo, fill='white', alpha=0.3)))
