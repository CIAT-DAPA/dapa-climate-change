...
# gcmList    <- c("bcc_csm1_1", "bcc_csm1_1_m","csiro_mk3_6_0", "gfdl_cm3", "ipsl_cm5a_lr", "miroc_miroc5", "ncc_noresm1_m")
gcmList <- c("miroc_esm", "miroc_esm_chem", "mri_cgcm3", "mohc_hadgem2_es", "nimr_hadgem2_ao")
    ...
    lapply(1:length(rcpList), function(k){
      ...
      lapply(1:length(gcmList), function(l){
        ...
        if(length(list.files(path=countyDir, recursive=TRUE))==14){
          ...
          NDWS <- mclapply(1:ncells, FUN=NDWSProcess, mc.cores=1)
