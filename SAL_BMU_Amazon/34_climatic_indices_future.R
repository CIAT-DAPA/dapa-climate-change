...
source('/mnt/workspace_cluster_9/Ecosystem_Services/SAL-project/WORK_PACKAGES/WP2/00_scripts/calc_risk_indices_modified.R')
# source('Z:/WORK_PACKAGES/WP2/00_scripts/calc_risk_indices_modified.R')
...
# gcmList    <- c("bcc_csm1_1", "bcc_csm1_1_m","csiro_mk3_6_0", "gfdl_cm3", "ipsl_cm5a_lr", "miroc_miroc5", "ncc_noresm1_m", "miroc_esm", "miroc_esm_chem", "mri_cgcm3")
gcmList <- c("mohc_hadgem2_es", "nimr_hadgem2_ao")

# years_analysis <- paste('y', 1981:2005, sep='')
iDir <- "/mnt/workspace_cluster_9/Ecosystem_Services/SAL-project/WORK_PACKAGES/WP2/06_Clustering_analyses"
# iDir <- "Z:/WORK_PACKAGES/WP2/06_Clustering_analyses"
inputDir <- '/mnt/workspace_cluster_9/Ecosystem_Services/SAL-project/WORK_PACKAGES/WP2/06_Clustering_analyses/data/bc_0_25deg_amz'
# inputDir <- "Z:/WORK_PACKAGES/WP2/06_Clustering_analyses/data/bc_0_25deg_amz"
outputDir <- '/mnt/workspace_cluster_9/Ecosystem_Services/SAL-project/WORK_PACKAGES/WP2/06_Clustering_analyses/results/climatic_indices/future'
# outputDir <- "Z:/WORK_PACKAGES/WP2/06_Clustering_analyses/results/climatic_indices/future"
region <- "amz"
  ...
  lapply(1:length(periodList), function(j){
    ...
    lapply(1:length(rcpList), function(k){
      ...
      lapply(1:length(gcmList), function(l){
        ...
        countyDir <- paste(inputDir, '/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', county), sep='')
        if(length(list.files(path=countyDir, recursive=TRUE))==14){
          
          cat('Loading raster mask for:', gsub(pattern=' ', replacement='_', county), '\n')
          countyMask <- raster(paste(iDir , "/data/", region, "_regions_rst/", gsub(pattern=' ', replacement='_', county), "_base_lr.tif", sep=""))
          ...
          load(paste(iDir, '/data/input_tables_lr/', gsub(pattern=' ', replacement='_', county), '/soil/soil_data.RData', sep=''))
          ...
          NDWS <- mclapply(1:ncells, FUN=NDWSProcess, mc.cores=24)
          ...
          # Created function
          GSEASProcess <- function(p){ # By pixel
            
            daysList <- Reduce(intersect, list(colnames(tmaxAll[,-c(1:3)]), colnames(tminAll[,-c(1:3)]),
                                               colnames(precAll[,-c(1:3)]), colnames(dswrfAll[,-c(1:3)])))
            
            out_all <- soil[which(soil$cellID==pixelList[p]), c('cellID', 'lon', 'lat')]
            out_all <- do.call("rbind", replicate(length(daysList), out_all, simplify=FALSE))
            out_all$SRAD <- as.numeric(dswrfAll[which(dswrfAll$cellID==pixelList[p]), match(daysList, colnames(dswrfAll))])
            out_all$TMIN <- as.numeric(tminAll[which(tminAll$cellID==pixelList[p]), match(daysList, colnames(tminAll))])
            out_all$TMAX <- as.numeric(tmaxAll[which(tmaxAll$cellID==pixelList[p]), match(daysList, colnames(tmaxAll))])
            out_all$RAIN <- as.numeric(precAll[which(precAll$cellID==pixelList[p]), match(daysList, colnames(precAll))])
            rownames(out_all) <- daysList
            
            if(gcmList[l] == "mohc_hadgem2_es" || gcmList[l] == "nimr_hadgem2_ao"){
              out_all <- out_all[!(day(as.Date(rownames(out_all),format='%Y-%m-%d'))=="29" & month(as.Date(rownames(out_all),format='%Y-%m-%d')) == 2),]
              out_all <- out_all[complete.cases(out_all), ]
            } else {
              out_all <- out_all[!(day(rownames(out_all))=="29" & month(rownames(out_all))==2),]  
            }
                        
            soilcp <- soil[which(soil$cellID==pixelList[p]), 'soilcp']
            ...
            if(!is.na(soilcp)){
              ...
              # Run process by year
              lgp_year_pixel <- lapply(1:length(years_analysis), function(y){
                
                # Subsetting by year
                watbal_year <- watbal_loc[year(rownames(watbal_loc))==gsub(pattern='y', replacement='', years_analysis[y]),]
                ...
                if(!sum(!is.na(runsDF$Lengths[runsDF$Condition==1] < 5)) == length(runsDF$Lengths[runsDF$Condition==1])){
                  ...
                  growingSeason <- lapply(1:length(LGP_seq), function(g){
                    ...
                    results <- data.frame(cellID=pixelList[p], year=gsub(pattern='y', replacement='', years_analysis[y]), gSeason=g, SLGP=LGP_ini, LGP=LGP)
                    return(results)
                  ...
                  growingSeason <- data.frame(cellID = pixelList[p], year = gsub(pattern='y', replacement='', years_analysis[y]), gSeason = 1:2, SLGP = NA, LGP = NA)
              ...
              return(cat('It is probable that pixel:', pixelList[p], 'is a water source\n'))
            }
          ...
          GSEAS <- foreach(p=1:ncells) %dopar% {
            GSEASProcessCMP(p)
          }
          ...
          if(season == 'first'){
            ...
            if(!file.exists(seasonDir)){dir.create(seasonDir, recursive = TRUE)}
