...
for (yr in years){
  ...
  oPlot <- paste0(oDir, '/skill_', yr, '.tif') 
  ...
  par(mar=c(4,4,2,1))
  boxplot(id_tmax$RMSE.TEST/10 ~ id_tmax$MONTH,xlab='Month',ylab='RMSE (ºC)',pch=20,cex=1.1,ylim=c(0,1), naMonth=(mth_lb), cex.axis=0.9, col="gray90")
  ...
  # tiff('fig-3f-rmse_tmean_monthly.tif',width=600, height=600,pointsize=8,compression='lzw',res=150)
  par(mar=c(4,4,2,1))
  boxplot(id_tmin$RMSE.TEST/10 ~ id_tmin$MONTH,xlab='Month',ylab='RMSE (ºC)',pch=20,cex=1.1,ylim=c(0,1), naMonth=(mth_lb), cex.axis=0.9, col="gray90")
  # text(x=11.9, y=0.98,"F", pos=4, cex=1.1.4)
  grid()
