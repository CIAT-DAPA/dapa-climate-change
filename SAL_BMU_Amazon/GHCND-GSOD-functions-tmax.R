...
getDataGHCN <- function(id,year,data.dir,base.mat) {
  ...
  if (file.exists(fName)) {
    ...
    if (nrow(stData)!=0) { #do only if there is data for that year
      tmax <- apply(base.mat,1,searchData,"D",stData) #get actual data
      ...
      base.mat$TMAX <- tmax; base.mat$M <- mflag; base.mat$Q <- qflag; base.mat$S <- sflag
    }
  }
  return(base.mat)
}
...
####################################################################################
getMthDataGSOD <- function(ogdir, st_ids, ogdir_mth, ogdir_30yr_avg){
  ...
  for (id in st_ids){
    ...
    files <- list.files(ogdir, pattern=id)
    
    cat("Converting to monthly GSOD ", id, "\n")
    
    # Station with more than 15 years of data
    if (length(files) >= 15){
      ...
      st_var <- as.data.frame(matrix(NA, nrow(st_data), 7))
      
      st_var[,1] <- paste(st_data$ID,sep="")
      st_var[,2] <- st_data$YEAR
      st_var[,3] <- st_data$MONTH
      st_var[,4] <- st_data$DAY
      st_var[,5] <- as.numeric(st_data$TMAX)
      st_var[,6] <- as.numeric(st_data$TMIN)
      st_var[,7] <- as.numeric(st_data$RAIN)
      
      names(st_var) <- c("Code", "Year", "Month", "Day", "Tmax", "Tmin", "Prec")

      st_mth_prec <- aggregate(st_var$Prec, by=list(Month=st_var$Month, Year=st_var$Year), sum_var)
      st_mth_temp <- aggregate(st_var[,5:6], list(Month=st_var$Month, Year=st_var$Year),avg_var)
      
      st_mth <- cbind(st_mth_temp, "Prec"=st_mth_prec[,length(st_mth_prec)])
      write.csv(st_mth, paste0(ogdir_mth, "/", id, ".csv"), row.names=F)
      
      st_30yr_avg <- aggregate(st_mth[,3:5], by=list(Month=st_mth$Month), avg_var)
      
      clim_tmax <- rbind(clim_tmax, cbind(id, t(st_30yr_avg$Tmax))) 
      clim_tmin <- rbind(clim_tmin, cbind(id, t(st_30yr_avg$Tmin))) 
      clim_prec <- rbind(clim_prec, cbind(id, t(st_30yr_avg$Prec))) 
  ...
  names(clim_tmax) <- c("Station", mths)
  names(clim_tmin) <- c("Station", mths)
  names(clim_prec) <- c("Station", mths)
  
  write.csv(clim_tmax, paste0(ogdir_30yravg, "/gsod_30yravg_tmax.csv"), row.names=F)
  write.csv(clim_tmin, paste0(ogdir_30yravg, "/gsod_30yravg_tmin.csv"), row.names=F)
  write.csv(clim_prec, paste0(ogdir_30yravg, "/gsod_30yravg_prec.csv"), row.names=F)
