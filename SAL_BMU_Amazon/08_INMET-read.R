...
daily_qc <- function(var="prec", bDir = "S:/observed/weather_station/col-ideam/daily-raw", oDir = "Z:/DATA/WP2/01_Weather_Stations/COL"){
  ...
  if (var == "prec"){ 
    ...
    varmod ="Umidade.Relativa.Media"
  }
  
  # List all txt files
  rutOrigen <- paste0(bDir) 
  files <- list.files(rutOrigen, pattern="\\.txt$")
  
  # Set period
  # x = seq(as.Date("2030/01"), as.Date("2059/01"), "moths") 
  x=seq(as.Date("1961/1/1"), as.Date("2015/12/31"), "days")
  
  # Date format
  fechas = format(x,"%Y%m%d")
  fechas = cbind.data.frame("Date"=fechas,"NA")
  
  datosprecip = as.data.frame(matrix(NA,nrow(fechas),length(files)))
  
  for (i in 1:length(files)){
    
    st <- read.table(paste(rutOrigen,"/", files[i],sep=""), header=T, sep=" ")
    ...
    Data_qc = qc(Datos)
    colnames(Data_qc) <- c("Date", "Value")
    
    
    # Merge daily data
    final = merge(fechas,Data_qc,by="Date",all.x=T)
    datosprecip[,i]=final[,3]
...
clim_calc <- function(var="prec",  bDir = "Z:/DATA/WP2/01_Weather_Stations/COL", oDir = "Z:/DATA/WP2/01_Weather_Stations/COL", st_loc="S:/observed/weather_station/col-ideam/stations_names.csv", sY=1981, fY=2010){
  ...
  climData <- cbind(join$Station, "INMET", join$Station, join$Name, "BRAZIL", join$Lon, join$Lat, join$Alt, join[which(colnames(join) %in% mths)], 30)
