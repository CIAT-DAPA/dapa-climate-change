...
#   shff_dt_yr <- shff_data[[varList[[i]]]]
  ...
  agmerra_county <- lapply(1:nrow(countyList), function(j)
  {
    ...
    template.shp <- raster(paste(iDir, '/data/', region, '_regions_rst/', gsub(pattern=' ', replacement='_', countyList$County[[j]]), '_base_lr.tif', sep=''))
...
#       data_year <- raster::stack(shff_dt_yr[grep(pattern=year[[k]], x=shff_dt_yr)])
      ...
      if(!file.exists(paste(iDir, '/data/', region, '_regions_rst/', region, '_base_lr.tif', sep=''))){
        ...
        writeRaster(kenya_shp, paste(iDir, '/data/', region, '_regions_rst/', region, '_base_lr.tif', sep=''), format='GTiff', overwrite=TRUE)
      } else {
        cat('Shapefile rasterized for:', countyList$County[[i]], 'exists. Read it\n')
        kenya_shp <-raster(paste(iDir, '/data/', region, '_regions_rst/', region, '_base_lr.tif', sep=''))
      }
      
      # Clipping rasters according to Kenya's extent
      data_year_cr_rs <- mask(crop(data_year, extent(kenya_shp)), kenya_shp)
      ...
      options(fftempdir = rTempDir)

      
      if(LeapYear==TRUE){ # Cases where we have leap years
        temp.dt <- ff(1, dim=c(nrow(rasterToPoints(template.shp)), 369), vmode="double")
        ...
        temp.dt <- ff(1, dim=c(nrow(rasterToPoints(template.shp)), 368), vmode="double")
    ...
    counDir <- paste(iDir, '/data/input_tables_lr/', gsub(pattern=' ', replacement='_', countyList$County[[j]]), "/", varList[[i]], sep='')
    if(!file.exists(counDir)){ dir.create(counDir) } else { cat('County folder exists\n') }
...
lapply(1:nrow(countyList), function(i)
{
  ...
  countyDir <- paste(iDir, '/data/input_tables_lr/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), sep='')
