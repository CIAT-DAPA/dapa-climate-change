...
source('/mnt/workspace_cluster_9/Ecosystem_Services/SAL-project/DATA/WP2/00_scripts/calc_risk_indices_modified.R')
...
inputDir <- paste0(iDir, '/data/input_tables_lr')
outputDir <- paste0(iDir, '/results/climatic_indices/historical')
...
calc_climIndices <- function(county='Bomet', season='first'){
  ...
  if(!file.exists(paste0(outputDir, '/', season, '_season/', gsub(pattern=' ', replacement='_', county, fixed=TRUE), '_', season, '_season.RData'))){

    if(file.exists(countyDir))
    {
      indexes <- paste(outputDir, '/', season, '_season/', gsub(pattern=' ', replacement='_', county, fixed=TRUE), '_', season, '_season.RData', sep='')
      ...
      }, mc.cores=length(years_analysis))
      ...
      dswrfAll <- agmerra_year; rm(agmerra_year)
      ...
      tmaxAll <- agmerra_year; rm(agmerra_year)
      ...
      tminAll <- agmerra_year; rm(agmerra_year)
      ...
      if(season == 'first'){
        ...
        if(!file.exists(seasonDir)){dir.create(seasonDir, recursive = T)}
