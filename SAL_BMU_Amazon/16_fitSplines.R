...
# 
# setwd("Z:/DATA/WP2/00_scripts")
# anuDir <- "Z:/DATA/WP2/00_scripts/anu/Anuspl43/bin"
# stDir <- "Z:/DATA/WP2/01_Weather_Stations/MERGE/climatology/1971_2000"
# rDir <- "Z:/DATA/WP2/00_zones"
# oDir <- "D:/cenavarro/lat-bmu/interpolation_tps_30s/1971_2000"
# train.per <- 0.80
# vn <- "tmin"
# tile <- 1
# ntiles <- 1
...
splineFitting <- function(anuDir="/anu/Anuspl43/bin", stDir, rDir, oDir, fold, train.per=0.85, vn="rain", ntiles=1, unix=F, suffix="some", srcDir="/scripts") {
  
  source("writeDatFile.R"); source("createFitFile.R"); source("createValFile.R"); source("createPrjFile.R"); source("accuracy_mod.R")
  ...
  for (tile in 1:ntiles) {
    ...
    if (!file.exists(paste(vn, "-status.anu", sep=""))) {
      ...
      write.csv(st.sel.10y, paste(tDir, "/", vn, "_", suffix,".csv", sep=""))
      ...
      st.sel.10y <- read.csv(paste(tDir,"/", vn, "_", suffix,".csv",sep=""))
