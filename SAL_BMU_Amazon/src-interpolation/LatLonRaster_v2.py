...
if len(sys.argv) < 5:
	os.system('cls')
	print "\n Too few args"
	print "   - ie: python LatLonRaster_v2.py S:\observed\gridded_products\srtm\Altitude_30s\alt D:\cenavarro\lat-bmu\region\5km_i amz 1"
	sys.exit(1)
...
OutAltRes = dirout + "\\alt"
OutAlt = dirout + "\\altitude"
if not arcpy.Exists(OutAlt):
	arcpy.Clip_management(dem, "-80 -12 -72 3", OutAltRes)
...
if not arcpy.Exists(dirout + "\\lon-prj-" + region + ".asc"):
	...
	diroutasc = dirout + "\\tile-1" 
	if not os.path.exists(diroutasc):
		os.system("mkdir " + diroutasc)
	
	gp.RasterToASCII_conversion(OutAlt, diroutasc + "\\altitude.asc")
	gp.RasterToASCII_conversion(OutRaster2, diroutasc + "\\latitude.asc")
	gp.RasterToASCII_conversion(OutRaster3, diroutasc + "\\longitude.asc")
