...
#     }
    ...
    for (mth in 1:12) {
      ...
      y.meas <- as.numeric(fit.vals[,p1])
      y.pred <- as.numeric(fit.vals[,p2])
      ...
      re <- abs(y.meas - y.pred)/y.meas
      re <- mean(re)
      
      #Output data frame
      out.fit <- data.frame(MONTH=mth, R2=rsq, P.VAL=pval, RMSE=rmse, RE=re)
  ...
  if (!is.null(testMx)) {
    ...
    for (mth in 1:12) {
      ...
      #Output data frame
      out.test <- data.frame(MONTH=mth, R2=rsq, P.VAL=pval, RMSE=rmse, RE=re)
  ...
  if (!is.null(testMx) & !is.null(trainMx)) {
    outMx <- data.frame(MONTH=otMx.fit$MONTH, R2.FIT=otMx.fit$R2, P.VAL.FIT=otMx.fit$P.VAL, RMSE.FIT=otMx.fit$RMSE, RE.FIT=otMx.fit$RE,
    R2.TEST=otMx.test$R2, P.TEST.FIT=otMx.test$P.VAL, RMSE.TEST=otMx.test$RMSE, RE.TEST=otMx.test$RE)
    ...
    outMx <- data.frame(MONTH=otMx.test$MONTH, R2.TEST=otMx.test$R2, P.TEST.FIT=otMx.test$P.VAL, RMSE.TEST=otMx.test$RMSE, RE.TEST=otMx.test$RE)
