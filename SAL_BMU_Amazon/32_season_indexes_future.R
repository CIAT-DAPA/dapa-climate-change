  ...
  lapply(1:length(periodList), function(j){
    ...
    lapply(1:length(rcpList), function(k){
      ...
      lapply(1:length(gcmList), function(l){
        ...
        precFile <- paste(iDir, '/data/bc_0_25deg_amz/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '/prec/bc_qmap_prec_', periodList[j],'.RData', sep='')
        
        if(file.exists(precFile)){
          
          t1 <- paste(iDir, '/data/bc_0_25deg_amz/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '/prec/bc_qmap_prec_', periodList[j], '_fs_wet_days.RData', sep='')
          t2 <- paste(iDir, '//data/bc_0_25deg_amz/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '/indx_fs_wet_days.RData', sep='')
          ...
          if(!file.exists(t1) & !file.exists(t2)){
            ...
            if(gcmList[l] == "mohc_hadgem2_es" || gcmList[l] == "nimr_hadgem2_ao"){
              years = sort(unique(unlist(lapply(strsplit(colnames(gcmFutBC[,-c(1:3)]), "-"), `[[`, 1))))
              ndays = 161
            } else {
            ...
            chirps_wet_days <- mclapply(1:length(years), function(m)
            {
              ...
              if(gcmList[l] == "mohc_hadgem2_es" || gcmList[l] == "nimr_hadgem2_ao"){
                df <- gcmFutBC[,c(1:3, which(unlist(lapply(strsplit(colnames(gcmFutBC[,-c(1:3)]), "-"), `[[`, 1))==years[m])+3)]
              }else{
              ...
              {
                first_season <- rsum.lapply(x=as.numeric(df[k,])[-c(1:3)][1:ndays], n=100)
                sum.ind <- cumulative.r.ind(first_season)[[which.max(cumulative.r.sum(first_season))]]
                df_upd <- data.frame(t(c(as.numeric(df[k,])[1:3], as.numeric(df[k,])[-c(1:3)][1:ndays][sum.ind])))
            ...
            save(chirps_wet_days, file=paste(iDir, '/data/bc_0_25deg_amz/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '/prec/bc_qmap_prec_', periodList[j], '_fs_wet_days.RData', sep=''))
            save(indexs_wet_days, file=paste(iDir, '/data/bc_0_25deg_amz/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '/indx_fs_wet_days.RData', sep=''))
    ...
    lapply(1:length(rcpList), function(k){
      ...
      lapply(1:length(gcmList), function(l){
        ...
        if(file.exists(precFile)){
          ...
          t1 <- paste(iDir, '/data/bc_0_25deg_amz/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '/prec/bc_qmap_prec_', periodList[j], '_ss_wet_days.RData', sep='')
          t2 <- paste(iDir, '/data/bc_0_25deg_amz/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '/indx_ss_wet_days.RData', sep='')
          ...
          if(!file.exists(t1) & !file.exists(t2)){
            ...
            chirps_wet_days <- mclapply(1:length(years), function(m)
            {
              ...
              {
                second_season <- rsum.lapply(x=as.numeric(df[k,])[-c(1:3)][ndays:length(as.numeric(df[k,])[-c(1:3)])], n=100)
                sum.ind <- cumulative.r.ind(second_season)[[which.max(cumulative.r.sum(second_season))]]
                df_upd <- data.frame(t(c(as.numeric(df[k,])[1:3], as.numeric(df[k,])[-c(1:3)][ndays:length(as.numeric(df[k,])[-c(1:3)])][sum.ind])))
                names(df_upd) <- c('cellID', 'lon', 'lat', paste('d', 1:100, sep=''))
                
                sum.ind_upd <- data.frame(t(c(as.numeric(df[k,])[1:3], (ndays:length(as.numeric(df[k,])[-c(1:3)]))[sum.ind])))
                names(sum.ind_upd) <- c('cellID', 'lon', 'lat', paste('d', 1:100, sep=''))
            ...
            save(chirps_wet_days, file=paste(iDir, '/data/bc_0_25deg_amz/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '/prec/bc_qmap_prec_', periodList[j], '_ss_wet_days.RData', sep=''))
            save(indexs_wet_days, file=paste(iDir, '/data/bc_0_25deg_amz/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '/indx_ss_wet_days.RData', sep=''))
  ...
  lapply(1:length(periodList), function(j){
    ...
    lapply(1:length(rcpList), function(k){
      ...
      lapply(1:length(gcmList), function(l){
        ...
        countyDir <- paste(iDir, '/data/bc_0_25deg_amz/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), sep='')
            ...
            if(length(list.files(paste(countyDir, '/', varList[m], sep=''), recursive=TRUE))>0){
              ...
              if(length(list.files(paste(countyDir, '/', varList[m], sep=''), recursive=TRUE))==1){
                ...
                ch2014_var_wet_days <- foreach(n=1:length(frs_wet_season)) %dopar% {
                  ...
                  yr <- as.numeric(gsub(pattern='y', replacement='', names(frs_wet_season)[n]))
...
#                     for(o in 1:length(pixelList))
                    ...
                    if(length(which(df_frs$cellID==cell)) > 0){
                      ...
                      if(rcpList[k] == "rcp45" && gcmList[l] == "miroc_esm"){
                        indx_scn <- indx_scn - 1
                      }
                      
                      if(gcmList[l] == "mohc_hadgem2_es" || gcmList[l] == "nimr_hadgem2_ao"){
                        if (indx_scn[length(indx_scn)] > 358) {
                          indx_scn <- indx_scn - (indx_scn[length(indx_scn)] - 359) - 1
                        }
