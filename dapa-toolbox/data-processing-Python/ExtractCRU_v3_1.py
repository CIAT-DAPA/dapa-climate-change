	...
	for year in range (startyear, endyear + 1, 1):
		...
		for month in range (1, 12 + 1, 1):
			for variable in variablelist:
				...
				if not os.path.basename(dbf) == variable + "_" +  str(year) + "_1.dbf":
					InData = dirout + "\\" + variable  + "_" + str(year) + "_1.dbf"
					if str(month)  == "10" or str(month) == "11" or str(month) == "12":
					...
					os.remove(dbf)
		for variable in variablelist:
			os.rename(dirout + "\\" + variable  + "_" + str(year)+ "_1.dbf", dirout + "\\" +variable + "_extracts.dbf")	
	# Remove trash files	
