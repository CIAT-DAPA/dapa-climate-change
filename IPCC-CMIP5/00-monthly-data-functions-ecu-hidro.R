...
#         for (period in periodList) {
          ...
          endYear <- as.integer(period) + 19#24
          ...
          for (var in varList) {
            ...
            for (mth in monthList) {
              ...
              if (!file.exists(outNcAvg)){
                ...
                mthNc <- lapply(paste(mthDir, "/", staYear:2016, "/", var, "_", mth, ".nc", sep=""), FUN=raster)
                
                raster('T:/gcm/cmip5/raw/monthly/rcp26/lasg_fgoals_g2/r1i1p1/monthly-files/2016/prec_01.nc')
...
# ens="r1i1p1"
# basePer <- "1981_2005"
# oDir="G:/cenavarro/ecu-hidroelectrica/anomalies_cmip5_all_ecu"
# bbox <- "G:/cenavarro/ecu-hidroelectrica/02_baseline/_region/ecu0.asc"
# otp <- GCMAnomalies(rcp, baseDir, ens, basePer, oDir, bbox)

GCMAnomalies <- function(rcp='rcp26', baseDir="T:/gcm/cmip5/raw/monthly", ens="r1i1p1", basePer="1981_2005", oDir="G:/cenavarro/ecu-hidroelectrica/anomalies_cmip5", bbox="G:/cenavarro/ecu-hidroelectrica/02_baseline/_region/alt-prj-ecu.asc") {
        ...
        if (file.exists(curAvgDir)){
          ...
          for (var in varList) {
            ...
            for (mth in monthList) {
              ...
              outNcCut <- paste(oDirPer, "/", var, "_", mth, ".nc", sep="")
              outNcRes <- paste(oDirPerRes, "/", var, "_", mth, ".nc", sep="")
              
              if (!file.exists(outNcCut)) {
                system(paste("cdo sellonlatbox,",extbbox@xmin-3,",",extbbox@xmax+3,",",extbbox@ymin-2,",",extbbox@ymax+3," ", outNc, " ", oDirPer, "/", var, "_", mth, ".nc", sep=""))
                #system(paste("cdo sellonlatbox,",extbbox@xmin+10,",",extbbox@xmax,",",extbbox@ymin,",",extbbox@ymax," ", outNc, " ", oDirPer, "/", var, "_", mth, ".nc", sep=""))
