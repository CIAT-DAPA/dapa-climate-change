...
#################################################################################
#################################################################################
retrieve_header <- function(x,xy_loc,years) {
  #xy_loc <- hh_mill_xy[1,]
  #x <- wth_site
  #years <- 2011
  
  #site name and details
  lon <- xy_loc$lon; lat <- xy_loc$lat; elev <- -99
  
  #filename
  basename <- "P4NE"; ext=".WTH"; yri <- substr(paste(min(years)),3,4); nyrs <- sprintf("%02d",length(years))
  fname <- paste(basename,yri,nyrs,ext,sep="")
  
  #site details
  s_details <- data.frame(NAME=paste("Niger hhid ",xy_loc$hhid,sep=""),INSI="P4NE",LAT=lat,LONG=lon,
                          ELEV=elev,TAV=-99,AMP=-99,REFHT=2.00,WNDHT=2.00,FNAME=fname)
