...
mapMetric <- function(wd="",dataset="ghcn",gcm="",variable="rain",period="TTL",metric="R2.FORCED") {
  #Input folder stuff
  ...
  #Now load the stations locations
  wsLoc <- read.csv(paste(inpDir,"/",dataset,"_",variable,"_1961_1990_mean.csv",sep=""))
  wsLoc <- data.frame(ID=wsLoc$ID,LAT=wsLoc$LAT,LONG=wsLoc$LONG)
