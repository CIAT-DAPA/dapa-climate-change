...
compareWSR <- function(gcmDir=NULL, gcm=NULL, shpDir=NULL, stationDir=NULL, country=NULL, variable=NULL, divide=T, months=c(6,7,8), outDir=NULL, verbose=T) {
  ...
  if (!file.exists(paste(outDir, "/", lgname, sep=""))) {
    ...
    #Loading stations
    st <- read.dbf(paste(stationDir, "/wc_", st.varName, "_stations.dbf", sep=""))
