...
#Remove the paired printing option -OK!
#Do versioning
...
cat("\n")
cat("mapGCMFields(gcmList, drive, procdir, scenario, type, period, xn=-180, xx=180, yn=-90, yx=90, wt=5, worldshapefile, plt='temp') \n")
cat("\n")
cat("\n")
cat("Where: \n")
...
cat("*** [plt] Plotting variable, can be either 'prec' or 'temp' \n")
cat("*** [writeRasterFiles=T/F] logical, will write raster files for each model and the MMM if TRUE \n")
cat("\n")

mapGCMFields <- function(gcmList, drive, procdir, scenario, type, period, xn=-30, xx=30, yn=-40, yx=40, wt=5, worldshapefile, plt="temp", writeRasterFiles=T) {
	...
	#Verifying the plotting variable is valid. Number of panels (np) is always 1
	plt <- tolower(plt)
	if (!plt %in% c("temp","prec")) {
		stop('Plotting variable', plt, ' is not supported')
	} else {
		np <- 1
		if (plt == "prec") {
			varList <- c("prec")
	...
	#Calculating averages from monthly data, defining legend extent
	gcmctr <- 1
	...
	for (gcmctr in 1:totgcms) {
		if (plt == "temp") {
			...
			#Direction of the changes (do an if for stipples calculation only when dealing with anomalies)
			#Binary raster to indicate whether the direction coincides or not
			if (type == "anomalies") {
				negCells <- which(get(paste("p12r",gcmctr,sep=""))[] < 0 & p12m[] < 0)
				posCells <- which(get(paste("p12r",gcmctr,sep=""))[] > 0 & p12m[] > 0)
				equCells <- which(get(paste("p12r",gcmctr,sep=""))[] == 0 & p12m[] == 0)
				...
				if (gcmctr == 1) {
					stplList <- get(paste("stpl",gcmctr,sep=""))
				} else {
					stplList <- c(stplList, get(paste("stpl",gcmctr,sep="")))
				}
			}
		}
	}
	...
	if (plt == "prec" & type == "anomalies") {
		...
		names(stplMatrix) <- c("CELL", "LON", "LAT", "PER_MODELS")
		selRows <- which(stplMatrix$PER_MODELS >= 80)
		
		pts <- stplMatrix[selRows,]
	}
