...
outDir <- paste(cd,"/raster/gridded/summaries",sep="")
...
ha_rsi <- raster(paste(cd,"/raster/summaries/harea-mean.asc",sep=""))
ha_rso <- paste(outDir,"/harea-mean.asc",sep="")

tp_rsi <- raster(paste(cd,"/raster/summaries/tprod-mean.asc",sep=""))
