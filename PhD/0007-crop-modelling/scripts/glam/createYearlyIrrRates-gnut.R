...
hDir <- paste(bDir,"/model-runs/GNUT/harvested_area",sep="")
outYearDir <- paste(hDir,"/raster/yearly",sep="")
if (!file.exists(outYearDir)) {dir.create(outYearDir)}

pfx <- "H"
dataType <- "raw"
inyData <- read.table(paste(hDir,"/data/IND2-gnut.tab",sep=""),sep="\t",header=T)
outDataDir <- outYearDir
