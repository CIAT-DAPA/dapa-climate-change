...
############################################################
GLAM_optimise_loc <- function(GLAM_params,RUN_setup,sect="glam_param.ygp",param="YGP",n.steps=20,
                              iter=1,iratio=0) {
  param <- toupper(param)
...
####### except sowing date, RLL, DUL, SAT
############################################################
GLAM_optimise_glo <- function(GLAM_params,RUN_setup,sect="glam_param.ygp",param="YGP",n.steps=20,
                              iter=1,iratio=0) {
  param <- toupper(param)
  simset <- RUN_setup$SIM_NAME
  zone <- RUN_setup$ZONE
  ...
  for (i in 1:length(vals)) {
    ...
    names(pred) <- c("YEAR","LAT","LON","PLANTING_DATE","STG","RLV_M","LAI","YIELD","BMASS","SLA",
                     ...
                     "T_EVAP","TP_EVAP","T_TRANS","RLA","RLA_NORM","RAIN_END","DSW","TRADABS",
                     "DUR","VPDTOT","TRADNET","TOTPP","TOTPP_HIT","TOTPP_WAT","TBARTOT")
    y_p <- data.frame(YEAR=pred$YEAR,LAT=pred$LAT,LON=pred$LON,YIELD=pred$YIELD)
    rowcol <- y_p[which(y_p$YEAR == min(y_p$YEAR)),]
    rowcol$ROW <- 1; rowcol$COL <- 1:nrow(rowcol)
    rowcol$YIELD <- NULL; rowcol$YEAR <- NULL
    y_p$ROW <- rep(rowcol$ROW,times=length(unique(y_p$YEAR)))
    y_p$COL <- rep(rowcol$COL,times=length(unique(y_p$YEAR)))
    
    y_o <- read.fortran(yFile,format=c("3I4","F8"))
    names(y_o) <- c("YEAR","ROW","COL","OBS")
    odf <- merge(y_p,y_o,by=c("YEAR","ROW","COL"),all.x=T,all.y=F)
    names(odf)[6] <- "RFD"
    ...
    y_p$ROW <- rep(rowcol$ROW,times=length(unique(y_p$YEAR)))
    y_p$COL <- rep(rowcol$COL,times=length(unique(y_p$YEAR)))
    y_p$LAT <- NULL; y_p$LON <- NULL
    names(y_p)[2] <- "IRR"
    
    odf <- merge(odf,y_p,by=c("YEAR","ROW","COL"),all=T)
    odf <- merge(odf,iratio,by=c("YEAR","ROW","COL"),all=T)
    odf$IRATIO[which(odf$IRATIO > 1)] <- 1
    odf$PRED <- (1-odf$IRATIO)*odf$RFD + (odf$IRATIO)*odf$IRR
