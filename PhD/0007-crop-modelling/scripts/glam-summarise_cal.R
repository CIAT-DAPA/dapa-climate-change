...
pts <- list("sp.points", xycal, pch = 4, col = "black", cex=0.6, lwd=0.6,first=F)

tiffName <- paste(out_rs_dir,"/ygp.tif",sep="")
tiff(tiffName,res=300,compression="lzw",height=ht,width=wt)
spplot(rs,sp.layout=list(pts,wld,grli),col.regions=cols,
...
xysig <- cbind(x=out_all$X[which(out_all$CCOEF > 0 & out_all$PVAL <= 0.05)],
               y=out_all$Y[which(out_all$CCOEF > 0 & out_all$PVAL <= 0.05)])
xysig <- SpatialPoints(xysig)
pts <- list("sp.points", xysig, pch = 20, col = "black", cex=0.75, lwd=0.6,first=F)
