...
##################################################################
#### my version of the taylor diagram function (modified from plotrix package)
##################################################################
#R = correlation coefficient
#sd.r = SD of observations
#sd.f = SD of model values
#add = whether to draw new diagram or just add point
#col = color of points displayed
#pch = type of point to display
#pos.cor = whether to display only positive (TRUE) or all values of correlation (FALSE)
#xlab,ylab,main = labels of axes and main title
#show.gamma = whether to display st. dev. arcs around the reference point
#             (only for pos.cor=T)
#ngamma = number of gammas to display (default=3)
#gamma.col= color to use for the gamma arcs (only with pos.cor=T)
#sd.arcs = whether to display arcs along the st. dev. axes
#ref.sd = whether to display the arc representing the reference st. dev.
...
#normalise = whether to normalise models so that st. dev. of reference is 1
#mar = margins -only applies to pos.cor=T
#... = any other arguments passed to plot


taylorD <- function (R, sd.r, sd.f, add = FALSE, col = "red", pch = 19, pos.cor = TRUE, 
          xlab = "", ylab = "", main = "Taylor Diagram", show.gamma = TRUE, 
          ngamma = 3, gamma.col = 8, sd.arcs = 0, ref.sd = TRUE, maxsd=NA,
          grad.corr.lines = c(0.2, 0.4, 0.6, 0.8, 0.9), pcex = 1, 
          cex.axis = 1, normalize = TRUE, mar = c(5, 4, 6, 6), ...) {
  ...
  if (is.na(maxsd)) maxsd <- 1.5 * max(c(sd.f, sd.r))
  ...
  if (!add) {
    if (pos.cor) {
      ...
      if (show.gamma[1]) {
        ...
        for (gindex in 1:length(gamma)) {
          ...
          if (xcurve[labelpos[gindex]] > 0) 
            boxed.labels(xcurve[labelpos[gindex]], ycurve[labelpos[gindex]], gamma[gindex], border = FALSE)
        }
      }
      ...
      #E2 <- E^2 + Eprime^2
      if (add == FALSE) {
        if (is.na(maxsd)) {maxray <-round(1.5 * max(c(sd.f, sd.r)),0)} else {maxray <- maxsd}
        plot(c(-maxray, maxray), c(0, maxray), type = "n", asp = 1, bty = "n", 
             xaxt = "n", yaxt = "n", xlab = xlab, ylab = ylab, main = main, cex = cex.axis)
        discrete <- seq(180, 0, by = -1)
        listepoints <- NULL
        for (i in discrete) {
          listepoints <- cbind(listepoints, maxray * cos(i * pi/180), 
                               maxray * sin(i * pi/180))
        }
...
################################################################################
mean_summary_interannual_vi <- function(this_proc) {
  ...
  if (!file.exists(paste(oDir,"/",met,"-",seas,"-",vn,"-min_all_models.tif",sep=""))) {
    ...
    cruList <- paste(s_dir,"/",met,"-",vn,"-vi-CRU-",gcmList,".tif",sep="")
    wstList <- paste(s_dir,"/",met,"-",vn,"-vi-WST-",gcmList,".tif",sep="")
...
##############################################################################
summarise_interannual_vi <- function(this_proc) {
  ...
  for (seas in sList) {
    ...
    if (!file.exists(paste(odir_seas,"/mbr-",vn,"-",dset,"-",gcm,"_",ens,".tif",sep=""))) {
      #output base rasters
      vi_rs <- raster(rs)
...
##############################################################################
# get metrics for interannual variability skill assessment
##############################################################################
summarise_interannual <- function(this_proc) {
  ...
  for (seas in sList) {
    ...
    if (!file.exists(paste(odir_seas,"/mbr-",vn,"-",dset,"-",gcm,"_",ens,".tif",sep=""))) {
      ...
      for (iso in isoList) {
        ...
        wcells <- cellFromXY(rs,cbind(x=sel_data$LON,y=sel_data$LAT))
        rmse_rs[wcells] <- sel_data$RMSE
        ccoef_rs[wcells] <- sel_data$CCOEF
        mbr_rs[wcells] <- sel_data$MBR
      }
      
      rmse_rs <- writeRaster(rmse_rs,paste(odir_seas,"/rmse-",vn,"-",dset,"-",gcm,"_",ens,".tif",sep=""),format="GTiff",overwrite=T)
      ccoef_rs <- writeRaster(ccoef_rs,paste(odir_seas,"/ccoef-",vn,"-",dset,"-",gcm,"_",ens,".tif",sep=""),format="GTiff",overwrite=T)
      mbr_rs <- writeRaster(mbr_rs,paste(odir_seas,"/mbr-",vn,"-",dset,"-",gcm,"_",ens,".tif",sep=""),format="GTiff",overwrite=T)
    }
  }
}
...
##############################################################################
get_mean_climate_metrics_revised2 <- function(proc,mdDir,regions) {
...
#################################################################################
interannual_vi_revised <- function(this_proc) {
...
mean_climate_skill_revised <- function(this_proc) {
  ...
  oDir <- paste(outputDD,"/",reg,"/",iso,sep="") #create output directory
  if (!file.exists(oDir)) {dir.create(oDir,recursive=T)}
  procDir <- paste(oDir,"/x_rev.proc",sep="") #output directory for .proc files
  if (!file.exists(procDir)) {dir.create(procDir)}
  ...
  for (vid in 1:3) {
    ...
    if (!file.exists(procFil)) {
      ...
      if (length(fPres) != 0) {
        ...
        vn_wcl <- paste(vnList$WCL[vid]) #variable name
        sc_wcl <- scList$WCL[vid]
        oclWCL <- paste(oDir,"/cl_rev-WCL",sep="") #create output folder
        if (!file.exists(oclWCL)) {dir.create(oclWCL)} #create output folder
        if (!file.exists(paste(oclWCL,"/",vn_gcm,"_",gcm,"_",ens,".csv",sep=""))) {
          wcl_data <- stack(paste(clWCL,"/",vn_wcl,"_",1:12,".tif",sep="")) #load all worldclim data
          wcl_data <- crop(wcl_data,msk) #cut worldclim data to extent of country mask
          wcl_vals <- sapply(1:12,mean_high_res_stk,xy,wcl_data) #average of worldclim onto model grid
          wcl_vals <- wcl_vals[,c(12,1:11)] * sc_wcl #re-order and scale
          s_skill <- seasonal_skill_revised(wcl_vals,gcm_vals,calc_mean) #assess seasonal skill using the two matrices
          write.csv(s_skill,paste(oclWCL,"/",vn_gcm,"_",gcm,"_",ens,".csv",sep=""),row.names=F,quote=T) #write output
          rm(wcl_data); g=gc()
        }
        ...
        vn_cru <- paste(vnList$CL_CRU[vid]) #variable name
        sc_cru <- scList$CL_CRU[vid]
        oclCRU <- paste(oDir,"/cl_rev-CRU",sep="") #create output folder
        if (!file.exists(oclCRU)) {dir.create(oclCRU)} #create output folder
        if (!file.exists(paste(oclCRU,"/",vn_gcm,"_",gcm,"_",ens,".csv",sep=""))) {
          cru_data <- stack(paste(clCRU,"/",vn_cru,"_",1:12,".tif",sep="")) #load all cru data
          cru_data <- crop(cru_data,msk) #cut cru data to extent of country mask
          cru_vals <- sapply(1:12,mean_high_res_stk,xy,cru_data) #average of cru onto model grid
          cru_vals <- cru_vals[,c(12,1:11)] * sc_cru #re-order and scale
          s_skill <- seasonal_skill_revised(cru_vals,gcm_vals,calc_mean) #assess seasonal skill using the two matrices
          write.csv(s_skill,paste(oclCRU,"/",vn_gcm,"_",gcm,"_",ens,".csv",sep=""),row.names=F,quote=T) #write output
          rm(cru_data); g=gc()
        }
        ...
        sc_e40 <- scList$E40[vid]
        if (vn_e40 != "NA") {
          oclE40 <- paste(oDir,"/cl_rev-E40",sep="") #create output folder
          if (!file.exists(oclE40)) {dir.create(oclE40)} #create output folder
          if (!file.exists(paste(oclE40,"/",vn_gcm,"_",gcm,"_",ens,".csv",sep=""))) {
            e40_data <- stack(paste(clE40,"/climatology_data_",vn_e40,"/",vn_e40,"_",1:12,".tif",sep="")) #load all era40 data
            e40_data <- crop(e40_data,msk) #cut era40 data to extent of country mask
            ...
            e40_vals <- extract(e40_data,xy) #extract era40 data
            e40_vals <- e40_vals[,c(12,1:11)] * sc_e40 #re-order and scale
            s_skill <- seasonal_skill_revised(e40_vals,gcm_vals,calc_mean) #assess seasonal skill using the two matrices
            write.csv(s_skill,paste(oclE40,"/",vn_gcm,"_",gcm,"_",ens,".csv",sep=""),row.names=F,quote=T) #write output
            rm(e40_data); g=gc()
          }
        }
        ...
        vn_wst <- paste(vnList$CL_WST[vid])
        sc_wst <- scList$CL_WST[vid]
        oclWST <- paste(oDir,"/cl_rev-WST",sep="") #create output folder
        if (!file.exists(oclWST)) {dir.create(oclWST)} #create output folder
        if (!file.exists(paste(oclWST,"/",vn_gcm,"_",gcm,"_",ens,".csv",sep=""))) {
          wst_raw <- read.dbf(paste(clWST,"/wc_",vn_wst,"_stations.dbf",sep=""))
          wst_raw <- wst_raw[which(!is.na(wst_raw$NYEARS)),]
          ...
          wst_vals <- as.matrix(wst_data[,toupper(month.abb)])
          wst_vals <- wst_vals[,c(12,1:11)] * sc_wst #reorder and scale
          s_skill <- seasonal_skill_revised(wst_vals,gcm_vals,calc_mean) #assess seasonal skill using the two matrices
          write.csv(s_skill,paste(oclWST,"/",vn_gcm,"_",gcm,"_",ens,".csv",sep=""),row.names=F,quote=T) #write output
          rm(wst_raw); g=gc()
        }
...
seasonal_skill_revised <- function(obs_vals,gcm_vals,calc_mean=T) {
  ...
  #calculate individual season skill metrics
  djf_mx <- calc_metrics_revised(djf)
  mam_mx <- calc_metrics_revised(mam)
  jja_mx <- calc_metrics_revised(jja)
  son_mx <- calc_metrics_revised(son)
  ann_mx <- calc_metrics_revised(ann)
...
###calculate metrics for a given matrix of two columns
###revised version for NCC paper
calc_metrics_revised <- function(vals) {
  #remove any column with NA
  vals <- vals[which(!is.na(vals$GCM)),]
  vals <- vals[which(is.finite(vals$GCM)),]
  vals <- vals[which(!is.na(vals$OBS)),]
  ...
  if (nrow(vals) > 0) {
    ...
    mbr <- as.numeric(lfit$coefficients)
    rmse <- sqrt(sum((vals$OBS-vals$GCM)^2)/nrow(vals))
    prmse1 <- rmse/mean(vals$OBS)*100
    prmse2 <- rmse/mean(vals$GCM)*100
    mean_obs <- mean(vals$OBS)
    mean_gcm <- mean(vals$GCM)
  } else {
  ...
  out_df <- data.frame(CCOEF=ccoef,PVAL=pval,RSQ=rsq,MBR=mbr,RMSE=rmse,PRMSE1=prmse1,
                       PRMSE2=prmse2,mean_OBS=mean_obs,mean_GCM=mean_gcm,N=npts)
  return(out_df)
}
...
calc_metrics <- function(vals) {
  ...
  out_df <- data.frame(CCOEF=ccoef,PVAL=pval,RSQ=rsq,MBR=mbr,RMSE=rmse,N=npts)
  return(out_df)
}
