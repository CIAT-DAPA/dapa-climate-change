...
# Quantile-mapping bias correction function for Garissa and Tana River
BC_Qmap <- function(county="Tana_River", rcp="rcp26", gcm="bcc_csm1_1", var="prec", period="2021_2045"){
  ...
  if(!file.exists(paste(outDir, "/", gcm, "/", period, "/", rcp, "/", county, "/", var, "/bc_qmap_", var, "_", period, ".RData", sep=""))){
    ...
    countyMaskSample <- as.data.frame(rasterToPoints(countyMask))
    countyMaskSample$cellID <- cellFromXY(countyMask, xy=cbind(countyMaskSample[,c('x','y')]))
    countyMaskSample <- countyMaskSample[,c('cellID', 'x', 'y')]
    colnames(countyMaskSample) <- c('cellID', 'lon', 'lat')
    
    # Select a sample of 500 pixels from original raster
    set.seed(1235)
    sampleID <- sample(x=countyMaskSample$cellID, size=500, replace=FALSE)
    countyMaskSample <- countyMaskSample[match(sampleID, countyMaskSample$cellID),]
    rownames(countyMaskSample) <- 1:nrow(countyMaskSample)
    if(!file.exists(paste('/mnt/workspace_cluster_8/Kenya_KACCAL/data/input_tables/', county, '/', county, '_sampleCells.csv', sep=''))){
      write.csv(countyMaskSample, paste('/mnt/workspace_cluster_8/Kenya_KACCAL/data/input_tables/', county, '/', county, '_sampleCells.csv', sep=''), row.names=FALSE)
    }
    ...
    hist.dat.all <- hist.dat.all[as.numeric(na.omit(match(countyMaskSample[,'cellID'], hist.dat.all[,'cellID']))),]
    ...
    gcmDataProcess <- function(yr)
    {
      monthProcess <- function(mth)
      {
        ...
        gcmHist.dat <- gcmHist.dat[as.numeric(na.omit(match(countyMaskSample[,'cellID'], gcmHist.dat[,'cellID']))),]
    ...
    gcmDataFutureProcess <- function(yr)
    {
      monthProcess <- function(mth)
      {
        ...
        gcmFut.dat <- gcmFut.dat[as.numeric(na.omit(match(countyMaskSample[,'cellID'], gcmFut.dat[,'cellID']))),]
  ...
  lapply(1:length(rcpList), function(k){
    ...
    lapply(1:length(gcmList), function(j){
      ...
      lapply(1:length(varList), function(i){
        ...
        BC_Qmap(county="Tana_River", rcp=rcpList[[k]], gcm=gcmList[[j]], var=varList[[i]], period=periodList[[l]])
      })
    })
  })
})
...
lapply(1:length(periodList), function(l){
  ...
  lapply(1:length(gcmList), function(j){
    ...
    lapply(1:length(varList), function(i){
      cat('Processing variable:', varList[[i]], '\n')
      BC_Qmap(county="Makueni", rcp='rcp85', gcm=gcmList[[j]], var=varList[[i]], period=periodList[[l]])
    })
  })
})
