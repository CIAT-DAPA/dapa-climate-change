...
countyList <- countyList[-1,]; rownames(countyList) <- 1:nrow(countyList)
  ...
  lapply(1:length(periodList), function(j){
    ...
    lapply(1:length(rcpList), function(k){
      ...
      lapply(1:length(gcmList), function(l){
        ...
        indexes2 <- paste(outputDir, '/', gcmList[l], '/', periodList[j], '/', rcpList[k], '/', gsub(pattern=' ', replacement='_', county, fixed=TRUE), '_growingSeasons.RData', sep='')
        ...
        if(length(list.files(path=countyDir, recursive=TRUE))==14){
          ...
          outDir <- paste(outputDir, '/', gcmList[l], '/', periodList[j], '/', rcpList[k], sep='')
...
lapply(1:nrow(countyList), function(z){
    calc_climIndicesLGP(county=countyList$County[z])
})
