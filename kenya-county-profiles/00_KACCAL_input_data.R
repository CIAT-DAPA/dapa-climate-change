...
chirps_county <- lapply(1:nrow(countyList), function(i)
{
  ...
  counDir <- paste('/mnt/workspace_cluster_8/Kenya_KACCAL/data/input_tables/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '/prec', sep='')
...
ch2014_var <- lapply(1:length(varList), function(i)
{
  ...
  ch2014_county <- lapply(1:nrow(countyList), function(j)
  {
    ...
    counDir <- paste('/mnt/workspace_cluster_8/Kenya_KACCAL/data/input_tables/', gsub(pattern=' ', replacement='_', countyList$County[[j]]), '/', varList[[i]], sep='')
    if(!dir.exists(counDir)){ dir.create(counDir, recursive = T) } else { cat('County folder exists\n') }
...
chirps_county <- lapply(1:nrow(countyList), function(i)
{
  ...
  load(paste('/mnt/workspace_cluster_8/Kenya_KACCAL/data/input_tables/', gsub(pattern=' ', replacement='_', countyList$County[[i]]),'/prec/prec.RData', sep=''))
