...
g <- gc(); rm(list = ls())
# Use the directory where your scripts are stored
setwd("D:/ToBackup/repositories/cc-repo/dapa-climate-change/kenya-county-profiles")
source('./KACCAL_calc_risk_indices_modified.R')
...
countyList <- data.frame(County = 'Kajiado')
countyList$County <- as.character(countyList$County)

years_analysis <- paste('y', 1981:2005, sep='')
mainDir <- 'C:/Users/haachicanoy/Desktop'
inputDir <- paste0(mainDir, '/data/input_tables')
if(!dir.exists(inputDir)){dir.create(inputDir, recursive = T)}
# Inside inputDir put Kajiado folder
outputDir <- paste0(mainDir, '/results/climatic_indices/historical')
...
calc_climIndices <- function(county = 'Bomet', season = 'first'){
  ...
  if(dir.exists(countyDir))
  {
    ...
    countyMask <- raster(paste0(mainDir, "/data/Kenya_counties_rst/", gsub(pattern=' ', replacement='_', county), "_base.tif", sep=""))
...
load("//dapadfs/workspace_cluster_12/Kenya_KACCAL/results/climatic_indices/historical/first_season/Kajiado_first_season_2015.RData")
names(clim_indexes)
clim_indexes <- lapply(1:length(clim_indexes), function(i){
  z <- clim_indexes[[i]]
  z$Index <- names(clim_indexes)[i]
  return(z)
})
clim_indexes2 <- rbind_all(clim_indexes)
clim_indexes2 <- clim_indexes2 %>% dplyr::select(cellID, lon, lat, Index, `1981`:`2015`)
clim_indexes2$Average <- rowMeans(x = clim_indexes2[,5:ncol(clim_indexes2)], na.rm = T)

plot_func <- function(df, name) {
  ggplot(data = df, aes(x = lon, y = lat, fill = Average)) +
    geom_raster() +
    coord_equal() +
    theme_bw() +
