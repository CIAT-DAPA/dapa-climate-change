...
wrapClimInd_2015 <- lapply(1:nrow(countyList), function(i){
  ...
  load(paste('//dapadfs/workspace_cluster_12/Kenya_KACCAL/results/climatic_indices/historical/first_season/', gsub(pattern=' ', replacement='_', countyList$County[[i]]),'_first_season_2015.RData', sep=''))
  first_season <- clim_indexes; rm(clim_indexes)
  load(paste('//dapadfs/workspace_cluster_12/Kenya_KACCAL/results/climatic_indices/historical/second_season/', gsub(pattern=' ', replacement='_', countyList$County[[i]]),'_second_season_2015.RData', sep=''))
  ...
  outDir <- paste('//dapadfs/workspace_cluster_12/Kenya_KACCAL/results/graphics/historical_trends/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), sep='')
...
lapply(1:nrow(countyList), function(i){
  ...
  lapply(1:length(indexList), function(j){
    ...
    outDir <- paste('//dapadfs/workspace_cluster_12/Kenya_KACCAL/results/graphics/historical_trends/', gsub(pattern=' ', replacement='_', countyList$County[i]), '/individualPlots', sep='')
    if(!dir.exists(outDir)){dir.create(outDir, recursive = T)}
...
# EPS to PNG
lapply(1:nrow(countyList), function(i){
  dir <- paste("/mnt/workspace_cluster_12/Kenya_KACCAL/results/graphics/historical_trends/", gsub(pattern = " ", replacement = "_", countyList$County[i]), "/individualPlots", sep = "")
    ...
    wrapFutClimInd <- lapply(1:length(periodList), function(i){
      ...
      wrapClimInd <- lapply(1:length(rcpList), function(j){
        ...
        wrapClimInd <- lapply(1:length(gcmList), function(k){
          ...
          load(paste('//dapadfs/workspace_cluster_12/Kenya_KACCAL/results/climatic_indices/future/', seasonList[h], '_season/', gcmList[k], '/', periodList[i], '/', rcpList[j], '/', gsub(pattern=' ', replacement='_', countyList$County[[g]]), '_', seasonList[h], '_season.RData', sep=''))
