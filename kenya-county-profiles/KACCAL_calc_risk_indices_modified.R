# Julian Ramirez-Villegas - H. Achicanoy
# UoL / CCAFS / CIAT
# Oct 2015 - Apr 2016
...
library(compiler)
tmeanCMP <- cmpfun(tmean)


### Total rain at year ***
calc_totrain <- function(RAIN){
  totrain <- sum(RAIN, na.rm=T)
  return(totrain)
}
library(compiler)
calc_totrainCMP <- cmpfun(calc_totrain)
...
library(compiler)
calc_wsdaysCMP <- cmpfun(calc_wsdays)
