...
#Modified April 29, 2014 to include the correct calculation of yearly time series for future simulations
...
suitCalc <- function(climPath='', climPath2='', yearly=T, sowDat='', harDat='', Gmin=90,Gmax=90,Tkmp=0,Tmin=9,Topmin=20,Topmax=34,Tmax=50,Rmin=150,Ropmin=300,Ropmax=400,Rmax=600, outfolder='', cropname='',ext=".asc",cropClimate=F) {
  minAdapt <- 0
  maxAdapt <- 1
  
  if (is.na(ext)) {ext <- ".tif"}
  ...
  for (i in 1:12) {
    if (!file.exists(paste(climPath, "/tmean_", i, ext, sep=""))) {
  	...
  	if (yearly) {
  	  if (!file.exists(paste(climPath2, "/tmean_", i, ext, sep=""))) {
  			stop("Error mean temperature for month ", i, ": file does not exist")
  		} else if (!file.exists(paste(climPath2, "/tmin_", i, ext, sep=""))) {
  			stop("Error min temperature for month ", i, ": file does not exist")
  		}
  	}
    if (!is.na(Rmin)) {
	  ...
	  if (yearly & !file.exists(paste(climPath2, "/prec_", i, ext, sep=""))) {
		stop("Error precipitation year+1 for month ", i, ": file does not exist")
	  }
    }
  }
  ...
  if (is.na(Rmin)) {
    ...
    climateStack2 <- stack(stack(paste(climPath2, "/tmean_", c(1:12), ext, sep="")), 
                           stack(paste(climPath2, "/tmin_", c(1:12), ext, sep="")))
  } else {
    climateStack <- stack(stack(paste(climPath, "/tmean_", c(1:12), ext, sep="")), 
                          stack(paste(climPath, "/tmin_", c(1:12), ext, sep="")), 
    ...
    climateStack2 <- stack(stack(paste(climPath2, "/tmean_", c(1:12), ext, sep="")), 
                           stack(paste(climPath2, "/tmin_", c(1:12), ext, sep="")), 
                           stack(paste(climPath2, "/prec_", c(1:12), ext, sep="")))
  }
  ...
  pSuitName <- paste(outfolder, "/", cropname, "_psuit.tif", sep="")
  tSuitName <- paste(outfolder, "/", cropname, "_tsuit.tif", sep="")
  fSuitName <- paste(outfolder, "/", cropname, "_suit.tif", sep="")
  cumPptName <- paste(outfolder, "/", cropname, "_gsrain.tif", sep="")
  meanTName <- paste(outfolder, "/", cropname, "_gstmean.tif", sep="")
  gsSuitName <- paste(outfolder, "/", cropname, "_gssuit.tif", sep="")
  ...
  gsSuitRaster <- raster(climateStack, 0)
  ...
  for (b in 1:bs$n) {
    ...
    if (length(validCells) > 0) {
      ...
      if (!is.na(Gavg) & yearly) {rowVals <- cbind(rowVals, extract(climateStack2,validCells))}
      if (is.na(Gavg)) {
    ...
    gsSuitRaster[validCells] <- gsSuitVec
    #pbStep(pb, b)
  }
  cat("\n")
  #pbClose(pb)
  ...
  gsSuitRaster <- writeRaster(gsSuitRaster, gsSuitName, format='GTiff', overwrite=TRUE)
  
  return(stack(pSuitRaster, tSuitRaster, fSuitRaster,cumPptRaster,meanTRaster,gsSuitRaster))
}
...
suitFun_sowFix <- function(dataPixel,params) {
  if(length(which(is.na(dataPixel)))!=0) {
    ...
    #year2
    if (params$yearly) {
      TavDataPixel <- c(TavDataPixel,dataPixel[37:48])
      TnDataPixel <- c(TnDataPixel,dataPixel[49:60])
      PptDataPixel <- c(PptDataPixel,dataPixel[61:72])
    }
	...
	if (!is.na(Rmin)) {
      ...
      #for yearly runs
      if (params$yearly) {nmonth <- 24} else {nmonth <- 12}
      ...
      for (i in 1:12) {
        ...
        if (!is.na(Rmin)) {
          ...
          if (params$yearly) {
            cumPpt[i] <- sum(PptDataPixel[start.month:end.month])
          } else {
            if (end.mth.p > 12) {
      ...
      for (i in 1:12) {
        ...
        for (j in start.month:end.month) {
          r.end.mth <- j
          if (!params$yearly) {if (r.end.mth > 12) {r.end.mth <- r.end.mth - 12}}
          r.nxt.mth <- r.end.mth + 1
    ...
    if (finFinSuit == 0) {FinGS <- 0} else {FinGS <- max(which(finSuit == max(finSuit)))}
        } else {
        precFinSuit <-NA
		precFinGS <-NA
    ...
    res <- c(precFinSuit, tempFinSuit, finFinSuit, precFinGS, tempFinGS,FinGS)
    return(res)
  }
}
