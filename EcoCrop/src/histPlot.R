...
histPlot <- function(dataset, gs=1, plotdir="", nb=20) {
	if (tolower(gs) %in% c("mean","max","min","mode")) {
		plotdata <- dataset[,grep(paste(toupper(gs), "_", sep=""), names(dataset))]
		plotdata <- plotdata[which(!is.na(plotdata[,1])),]
		
		pc <- 1
		for (pv in c("prec","tmean","tmin","tmax")) {
			jpeg(paste(plotdir, "/GS", toupper(gs), "_", pv, ".jpg", sep=""), quality=100, height=600, width=600)
			hist(plotdata[,pc], breaks=nb, xlab=pv, main=pv)
			dev.off()
			pc <- pc+1
		}
		jpeg(paste(plotdir, "/GS", toupper(gs), "_temp.jpg", sep=""), quality=100, height=600, width=600)
