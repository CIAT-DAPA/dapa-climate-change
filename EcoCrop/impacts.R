...
# src.dir <- "E:/EcoCrop-development/_scripts"
...
iMetrix <- function(csr, fsr, shp, oDir, chggrid=T, impact=T, classes=T) {
	...
	for (p in 1:nPol) {
		...
		if (impact) {
			...
			im.th <- impact(cu, fu, threshold=70) #impact for g50 mask
...
suitArea <- function(csr, shp) {
	...
	for (p in 1:nPol) {
		...
		as.th <- areaSuitable(cu, threshold=70) #impact for g50 mask
		asm <- c(as.nz, as.th) #merge both matrices
		asm <- cbind(CID=rep(p,times=length(asm)), COUNTRY=rep(cname,times=length(asm)), THRESH=c(0,70), AREA.SUIT=asm)		
